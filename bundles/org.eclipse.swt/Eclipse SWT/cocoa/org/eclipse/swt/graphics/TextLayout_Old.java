...
public TextLayout_Old(Device device) {
	...
	/*
	 * Bug in Cocoa. Adding attributes directly to a NSTextStorage causes output to
	 * the console and eventually a segmentation fault when printing on a thread
	 * other than the main thread. The fix is to add attributes to a separate
	...
	if (tabs != null && tabs.length > 0) {
	    ...
	    if (count == 1) {
		/**
		 * defaultTabWidth holds the tabstop in points. There are two conditions we need
		 * to consider if this value is 0 we need to use tabstop from tabs[]. Also in
		 * case of user setting only one tabstop we need to use the values from tab[]
		 * this can be determined by comparing tab[0] with defaultTabWidth
		 */
	...
	for (int i = 0; i < stylesCount - 1; i++) {
		...
		NSColor color = NSColor.colorWithDeviceRed(foreground.handle[0], foreground.handle[1],
			foreground.handle[2], 1);
		attrStr.addAttribute(OS.NSForegroundColorAttributeName, color, range);
	    }
	    Color background = style.background;
	    if (background != null && !background.isDisposed()) {
		NSColor color = NSColor.colorWithDeviceRed(background.handle[0], background.handle[1],
			background.handle[2], 1);
		attrStr.addAttribute(OS.NSBackgroundColorAttributeName, color, range);
	    }
	    if (style.strikeout) {
		attrStr.addAttribute(OS.NSStrikethroughStyleAttributeName,
			NSNumber.numberWithInt(OS.NSUnderlineStyleSingle), range);
		Color strikeColor = style.strikeoutColor;
		if (strikeColor != null && !strikeColor.isDisposed()) {
		    NSColor color = NSColor.colorWithDeviceRed(strikeColor.handle[0], strikeColor.handle[1],
			    strikeColor.handle[2], 1);
		...
		if (underlineStyle != 0) {
		    attrStr.addAttribute(OS.NSUnderlineStyleAttributeName, NSNumber.numberWithInt(underlineStyle),
			    range);
		    Color underlineColor = style.underlineColor;
		    if (underlineColor != null && !underlineColor.isDisposed()) {
			NSColor color = NSColor.colorWithDeviceRed(underlineColor.handle[0], underlineColor.handle[1],
				underlineColor.handle[2], 1);
		...
		NSTextAttachment attachment = ((NSTextAttachment) new NSTextAttachment().alloc())
			.initWithFileWrapper(null);
	...
	for (numberOfLines = 0, index = 0; index < numberOfGlyphs; numberOfLines++) {
		...
		final double realDescent = layoutManager.typesetter().baselineOffsetInLayoutManager(layoutManager,
			lineOffset);
	...
	try {
		...
		NSColor selectionColor = NSColor.colorWithDeviceRed(selectionBackground.handle[0],
			selectionBackground.handle[1], selectionBackground.handle[2], selectionBackground.handle[3]);
		...
		// TODO draw full selection for wrapped text and delimiter selection for hard
		// breaks
		if ((flags & (SWT.FULL_SELECTION | SWT.DELIMITER_SELECTION)) != 0
			&& (/* hasSelection || */ (flags & SWT.LAST_LINE_SELECTION) != 0)) {
		...
		for (int j = 0; j < stylesCount; j++) {
		    ...
		    drawUnderline = drawUnderline
			    && (j + 1 == stylesCount || !style.isAdherentUnderline(styles[j + 1].style));
			...
			if (drawUnderline) {
				...
				if (range.length > 0) {
				    long[] rectCount = new long[1];
				    long pArray = layoutManager.rectArrayForCharacterRange(range, range, textContainer,
					    rectCount);
				    NSRect rect = new NSRect();
				    ngc.handle.saveGraphicsState();
				    double baseline = layoutManager.typesetter()
					    .baselineOffsetInLayoutManager(layoutManager, lineStart);
				    double[] color = null;
					...
					case SWT.UNDERLINE_SQUIGGLE: {
					    ...
					    double squigglyY = Math.min(underlineY - squigglyHeight / 2,
						    lineBottom - squigglyHeight - 1);
					    float[] points = computePolyline((int) underlineX, (int) squigglyY,
						    (int) (underlineX + rect.width),
						    (int) (squigglyY + squigglyHeight));
			...
			if (drawBorder) {
				...
				if (range.length > 0) {
					...
					dashes = width != 0 ? NativeGC.LINE_DASH : GC.LINE_DASH_ZERO;
					break;
				    case SWT.BORDER_DOT:
					dashes = width != 0 ? NativeGC.LINE_DOT : GC.LINE_DOT_ZERO;
					break;
				    }
				    double[] lengths = null;
				    if (dashes != null) {
	...
	try {
	    ...
	    int baseline = (int) layoutManager.typesetter().baselineOffsetInLayoutManager(layoutManager,
