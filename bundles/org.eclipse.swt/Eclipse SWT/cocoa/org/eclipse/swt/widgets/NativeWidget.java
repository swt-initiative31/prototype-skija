 ...
 * @see #getStyle
 */
public NativeWidget(Widget parent, int style) {
	checkSubclass ();
	checkParent (parent);
	getWidget().style = style;
	getWidget().display = parent.display;
	reskinWidget ();
	notifyCreationTracker();
}
    ...
    void reskinWidget() {
	if ((getWidget().state & SKIN_NEEDED) != SKIN_NEEDED) {
	    getWidget().state |= SKIN_NEEDED;
	    getWidget().display.addSkinnableWidget(getWidget());
	}
    }
	...
	if (getWidget().eventTable == null)
	    getWidget().eventTable = new EventTable();
	getWidget().eventTable.hook(eventType, listener);
    }
	...
	getWidget().style &= ~SWT.MIRRORED;
	if ((getWidget().style & (SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT)) == 0) {
	    if (parent != null) {
		if ((parent.style & SWT.LEFT_TO_RIGHT) != 0)
		    getWidget().style |= SWT.LEFT_TO_RIGHT;
		if ((parent.style & SWT.RIGHT_TO_LEFT) != 0)
		    getWidget().style |= SWT.RIGHT_TO_LEFT;
	    }
	}
	getWidget().style = checkBits(getWidget().style, SWT.LEFT_TO_RIGHT, SWT.RIGHT_TO_LEFT, 0, 0, 0, 0);
    }
	...
	Display display = getWidget().display;
	if (display == null)
	    error(SWT.ERROR_WIDGET_DISPOSED);
	if (display.thread != Thread.currentThread())
	    error(SWT.ERROR_THREAD_INVALID_ACCESS);
	if ((getWidget().state & DISPOSED) != 0)
	    error(SWT.ERROR_WIDGET_DISPOSED);
    }
	...
	/*
	 * Since macOS 14 the clipsToBounds property of NSView has to be set to true See
	 * https://developer.apple.com/documentation/macos-release-notes/appkit-release-
	 * notes-for-macos-14
	 */
	...
	if (!isDisposed()) {
	    /*
	     * Feature in Cocoa. There are widgets that draw outside of the UI thread, such
	     * as the progress bar and default button. The fix is to draw the widget but not
	     * send paint events.
	     */
	...
	return getWidget().display.filters(eventType);
    }
	...
	checkWidget();
	return (getWidget().state & KEYED_DATA) != 0 ? ((Object[]) getWidget().data)[0] : getWidget().data;
    }
	...
	if ((getWidget().state & KEYED_DATA) != 0) {
	    Object[] table = (Object[]) getWidget().data;
	...
	return getWidget().eventTable.getListeners(eventType);
    }
	...
	if (getWidget().eventTable == null)
	    return false;
	return getWidget().eventTable.hooks(eventType);
    }
	...
	return (getWidget().state & DISPOSED) != 0;
    }
    ...
    boolean menuHasKeyEquivalent_forEvent_target_action(long id, long sel, long menu, long event, long target,
	    long action) {
	return true;
    }
    ...
    long outlineView_objectValueForTableColumn_byItem(long id, long sel, long outlineView, long tableColumn,
	    long item) {
	return 0;
    }
    ...
    boolean outlineView_shouldReorderColumn_toColumn(long id, long sel, long aTableView, long columnIndex,
	    long newColumnIndex) {
	return true;
    }
    ...
    boolean outlineView_shouldTrackCell_forTableColumn_item(long id, long sel, long table, long cell, long tableColumn,
	    long item) {
	return true;
    }

    void outlineView_willDisplayCell_forTableColumn_item(long id, long sel, long outlineView, long cell,
	    long tableColumn, long item) {
    }
    ...
    void outlineView_setObjectValue_forTableColumn_byItem(long id, long sel, long outlineView, long object,
	    long tableColumn, long item) {
    }
	...
	try (ExceptionStash exceptions = new ExceptionStash()) {
	    if ((getWidget().state & DISPOSE_SENT) == 0) {
		getWidget().state |= DISPOSE_SENT;
		try {
	    ...
	    if ((getWidget().state & RELEASED) == 0) {
		getWidget().state |= RELEASED;
		if (destroy) {
		    releaseParent();
		    releaseWidget();
		    destroyWidget();
    ...
    void releaseHandle() {
	getWidget().state |= DISPOSED;
	getWidget().display = null;
	destroyJNIRef();
    }
    ...
    void releaseWidget() {
	deregister();
	if (getWidget().display.tooltipTarget == getWidget())
	    getWidget().display.tooltipTarget = null;
	getWidget().eventTable = null;
	getWidget().data = null;
    }
	...
	if (getWidget().eventTable == null)
	    return;
	getWidget().eventTable.unhook(eventType, listener);
    }
	...
	if (getWidget().eventTable == null)
	    return;
	getWidget().eventTable.unhook(SWT.Dispose, listener);
    }
	...
	getWidget().display.sendEvent(getWidget().eventTable, event);
    }
	...
	if (getWidget().eventTable == null && !getWidget().display.filters(eventType)) {
	    return;
	}
	if (event == null)
	    event = new Event();
	...
	if (event.time == 0) {
	    event.time = getWidget().display.getLastEventTime();
	}
	if (send) {
	    sendEvent(event);
	} else {
	    getWidget().display.postEvent(event);
	}
    }

    boolean sendKeyEvent(NSEvent nsEvent, int type) {
	if ((getWidget().state & WEBKIT_EVENTS_FIX) != 0)
	    return true;
	 ...
	 * widget in the key events. If this happens, end the processing of the key by
	 * returning false.
	 */
	if (isDisposed())
	    return false;
	...
	if (WEBKIT_EVENTS_FIX_KEY.equals(data)) {
	    getWidget().state |= WEBKIT_EVENTS_FIX;
	    return;
	}
	...
	if (value != null) {
		...
		if (index == table.length) {
		    ...
		    getWidget().data = table = newTable;
		}
	    } else {
		table = new Object[3];
		table[0] = getWidget().data;
		getWidget().data = table;
		state |= KEYED_DATA;
	    }
	    table[index] = key;
	    table[index + 1] = value;
		...
		if (index != table.length) {
		    ...
		    if (length == 1) {
			getWidget().data = table[0];
			state &= ~KEYED_DATA;
		    } else {
			Object[] newTable = new Object[length];
			System.arraycopy(table, 0, newTable, 0, index);
			System.arraycopy(table, index + 2, newTable, index, length - index);
			getWidget().data = newTable;
		    }
		}
	    }
	}
	...
	return setInputState(event, null, type);
    }
	...
	if (keyLayout == 0) {
	    // KCHR keyboard layouts are no longer supported, so fall back to the basic but
	    // flawed
	...
	for (int modifiers : modifierTests) {
	    OS.UCKeyTranslate(keyLayout, nsKeyCode, keyAction, modifiers, keyboardType, 0, deadKeyState,
     ...
     * <ul>
     * <li>-[NSEvent keyCode] contains the physical key id, it doesn't change across
     * keyboard layouts.</li>
     * <li>-[NSEvent modifierFlags] is the modifiers (Cmd, Shift etc)</li>
     * <li>-[NSEvent characters] is the translated character with keyboard layout,
     * keyCode, modifiers taken into account</li>
     * <li>-[NSEvent charactersIgnoringModifiers] is similar to the previous one,
     * but ignores modifiers other than Shift</li>
     * </ul>
     * Let's see some examples: (chars=-[NSEvent characters], charsNoMods=-[NSEvent
     * charactersIgnoringModifiers])<br>
     * <table>
     * <tr>
     * <th>Layout</th>
     * <th>US key label</th>
     ...
     * <ul>
     * <li>In non-latin layouts such as Bulgarian, with Cmd and/or Opt mods,
     * -[NSEvent characters] contains a character good for keyboard shortcuts, but
     * without modifiers, it's not given anywhere</li>
     * <li>In some non-latin layouts, Opt modifier results in good value in
     * -[NSEvent characters] (see Bulgarian), but in others, it doesn't (see
     * Russian)</li>
     * <li>In a lot of layouts, Shift+letter gives capital letter</li>
     * <li>In U.S. layout, Shift+2 doesn't give 2 anywhere</li>
     * <li>On the contrary, in layouts such as ABC-AZERTY (used in French), digits
     * keys don't give digits anywhere unless Shift is pressed</li>
     * <li>For Opt+Cmd+O, the good character is present, but in U.S. it's reported
     * in -[NSEvent charactersIgnoringModifiers] while in Bulgarian it's reported in
     * -[NSEvent characters]</li>
     * </ul>
     *
     * macOS documentation, such as in -[NSResponder performKeyEquivalent:],
     * suggests to use -[NSEvent charactersIgnoringModifiers], but clealry this
     * isn't going to work.
     *
     * What macOS does itself in -[NSMenu performKeyEquivalent:] is a lot of
     * undocumented magic, where it also maps physical keys to other layouts such as
     * <ul>
     * <li>TISCopyCurrentASCIICapableKeyboardLayoutInputSource - a documented API
     * that returns "most-recently-used ASCII-capable keyboard"</li>
     * <li>TSMDefaultAsciiCapableKeyboardLayoutCopy - a non documented API whose
     * meaning is hinted in documentation for previous API, "default ASCII-capable
     * keyboard layout (chosen by Setup Assistant)"</li>
     * </ul>
     *
	...
	case SWT.LF: {
	    /*
	     * Feature in the Macintosh. When the numeric key pad Enter key is pressed, it
	     * generates '\n'. This is the correct platform behavior but is not portable.
	 ...
	 */
	OS.objc_msgSend(id, OS.sel_setClipsToBounds_, true);
	if (flag && getWidget().display.isPainting.containsObject(view)) {
	    NSMutableArray needsDisplay = getWidget().display.needsDisplay;
	    if (needsDisplay == null) {
		needsDisplay = (NSMutableArray) new NSMutableArray().alloc();
		getWidget().display.needsDisplay = needsDisplay = needsDisplay.initWithCapacity(12);
	    }
	 ...
	 */
	OS.objc_msgSend(id, OS.sel_setClipsToBounds_, true);
	if (getWidget().display.isPainting.containsObject(view)) {
	    NSMutableArray needsDisplayInRect = getWidget().display.needsDisplayInRect;
	    if (needsDisplayInRect == null) {
		needsDisplayInRect = (NSMutableArray) new NSMutableArray().alloc();
		getWidget().display.needsDisplayInRect = needsDisplayInRect = needsDisplayInRect.initWithCapacity(12);
	    }
    ...
    void tableView_setObjectValue_forTableColumn_row(long id, long sel, long aTableView, long anObject,
	    long aTableColumn, long rowIndex) {
    }

    boolean tableView_shouldReorderColumn_toColumn(long id, long sel, long aTableView, long columnIndex,
	    long newColumnIndex) {
	return true;
    }
    ...
    boolean tableView_shouldTrackCell_forTableColumn_row(long id, long sel, long table, long cell,
	    /* long */ long tableColumn, long rowIndex) {
	return true;
    }

    void tableView_willDisplayCell_forTableColumn_row(long id, long sel, long aTableView, long aCell, long aTableColumn,
	    long rowIndex) {
    }
    ...
    NSRange textView_willChangeSelectionFromCharacterRange_toCharacterRange(long id, long sel, long aTextView,
	    long oldSelectedCharRange, long newSelectedCharRange) {
	return new NSRange();
    }
    ...
    long toolbar_itemForItemIdentifier_willBeInsertedIntoToolbar(long id, long sel, long toolbar, long itemID,
	    boolean flag) {
	return 0;
    }
	...
	if (WidgetSpy.isEnabled) {
	    WidgetSpy.getInstance().widgetCreated(getWidget());
	}
    }

    void notifyDisposalTracker() {
	if (WidgetSpy.isEnabled) {
	    WidgetSpy.getInstance().widgetDisposed(getWidget());
	}
    }
