...
void createPopupShell(int year, int month, int day) {
	...
	popupCalendar = new DateTime (popupShell.wrap(), SWT.CALENDAR);
	if (font != null) popupCalendar.setFont (font);

	if (clickListener == null) {
		clickListener = event -> {
			if (event.widget instanceof Control c && event.widget != NativeDateTime.this.wrap()) {
				if (c.getShell() != popupShell.wrap()) {
					hideCalendar();
				}
			}
		};
