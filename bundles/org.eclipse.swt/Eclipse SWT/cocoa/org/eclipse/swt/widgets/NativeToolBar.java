...
NativeToolBar(NativeComposite parent, int style, boolean internal) {
...
static int checkStyle (NativeComposite parent, int style, boolean internal) {
	/*
	...
	*/
	...
	if ((style & SWT.SMOOTH) != 0) {
		if (parent instanceof NativeShell s) {
...
void createItem (NativeToolItem item, int index) {
	...
	if (itemCount == items.length) {
		NativeToolItem [] newItems = new NativeToolItem [itemCount + 4];
...
boolean translateTraversal (int key, NSEvent theEvent, boolean[] consume) {
	...
	while ((index = (index + offset + length) % length) != start) {
		NativeToolItem child = items [index];
		if (!child.isDisposed ()) {
			if (child.setFocus ()) {
				lastFocus = child;
				return false;
