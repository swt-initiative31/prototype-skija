...
public abstract class Scrollable extends Control {
	...
	private void layoutScrollbars() {
		System.out.println("layoutScrollbars called");
		Rectangle bounds;
		...
		if (content instanceof Composite composite) {
			...
			if (!computed.equals(current)) {
				System.out.println("Updating content size from " + current + " to " + computed);
				content.setSize(computed);
			}
			bounds = new Rectangle(0, 0, computed.x, computed.y);

		} else {
			bounds = content.getBounds();
			System.out.println("Content bounds: " + bounds);
		}
		if ((bounds.width == 0 || bounds.height == 0) && minSize != null) {
			System.out.println("Using minSize: " + minSize);
			bounds = new Rectangle(0, 0, minSize.x, minSize.y);
		}
		int barSize = 16;
		if (horizontalBar != null) {
			System.out.println("Setting horizontalBar bounds");
			horizontalBar.setCustomBounds(0, bounds.height - barSize, bounds.width - barSize, barSize);
		}
		if (verticalBar != null) {
			System.out.println("Setting verticalBar bounds");
			verticalBar.setCustomBounds(bounds.width - barSize, 0, barSize, bounds.height - barSize);
		}
		if (horizontalBar != null) {
			System.out.println("Setting horizontalBar max: " + bounds.width);
			horizontalBar.setMaximum(bounds.width);
			horizontalBar.setThumb(Math.min(clientArea.width, bounds.width));
		}
		if (verticalBar != null) {
			System.out.println("Setting verticalBar max: " + bounds.height);
			verticalBar.setMaximum(bounds.height);
			verticalBar.setThumb(Math.max(1, Math.min(clientArea.height, bounds.height)));
			System.out.println("Setting verticalBar thumb: " + Math.max(1, Math.min(clientArea.height, bounds.height)));
		}
		System.out.printf("content height = %d, client height = %d, scrollable range = %d%n", content.getSize().y,
				getClientArea().height, Math.max(0, content.getSize().y - getClientArea().height));
...
//			SkijaGC skijaGC = (SkijaGC) gc.innerGC;
//			io.github.humbleui.skija.Canvas canvas = skijaGC.getCanvas();
//
//			canvas.save();
...
//			if (horizontalBar != null)
//				((ScrollBar) horizontalBar).draw(canvas);
//			if (verticalBar != null)
//				((ScrollBar) verticalBar).draw(canvas);
//		});
//		updateContentReference();
	}
