...
public class Group extends Composite {
	...
	private Point textExtentCache = new Point(0, 0);
	private static boolean borderSet = false;
	...
	private void drawGroup(GC gc) {
		...
		int groupHeight = height - (inset * 5);
		if (borderSet) {
			/** Border has to be drawn using Skija once native border is disabled. */
			inset = (orientation == SWT.RIGHT_TO_LEFT) ? 4 : 0;
			groupWidth -= 1;
			groupHeight -= 4;
		}
		// Set shadow color
	...
	public void setFont(Font font) {
		checkWidget();
		super.setFont(font);
		textExtentCache = Drawing.executeOnGC(this, gc -> {
			gc.setFont(getFont());
