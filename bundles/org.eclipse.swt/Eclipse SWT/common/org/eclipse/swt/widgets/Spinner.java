...
public class Spinner extends Composite {
	...
	private static final class ButtonView extends Composite {
	...
	Text textView;
	NumberFormat textFormatter = NumberFormat.getInstance();
	ButtonView buttonView; // TODO (visjee) these should be 2 buttons
...
private void createButtons() {
	buttonView = new ButtonView(this, style);
}
...
void internalLayout(boolean changed) {
	...
	Point arrowSize = buttonView.computeSize(SWT.DEFAULT, height, changed);
	textView.setBounds(0, 0, width - arrowSize.x, height);
	buttonView.setBounds(width - arrowSize.x, 0, arrowSize.x, arrowSize.y);
}
...
int getSelectionText (boolean[] parseFail) {
	String string = textView.getSelectionText();
	try {
		int value;
		if (digits > 0) {
...
void releaseHandle () {
	super.releaseHandle();
	if (buttonView != null) buttonView.releaseHandle();
	if (textView != null) textView.releaseHandle();
	textFormatter = null;
	buttonView = null;
	textView = null;
}
...
void setSelection(int value, boolean setText, boolean notify) {
	if (setText) {
		...
		if (hooks (SWT.Verify) || filters (SWT.Verify)) {
			int length = (int) textView.getText().length();
