...
class DecorationsHandler implements Listener {
	...
	if ((item.getStyle() & SWT.CASCADE) != 0) {
	    new MenuWindow(decorations, item.getMenu().getItems(), new Point(rec.x, rec.y), e.gc).open();
	}
    }
	...
	gr.drawText(decorations.getText(), 4, 8);
	...
	return decorations.getDisplay().getSystemColor(SWT.COLOR_BLACK);
    }
	...
	if (withFocus) {
		...
		bgDecorationFocus = new Color(decorations.getDisplay(), new RGB(200, 200, 200));

	    return bgDecorationFocus;
	}

	if (bgDecorationNoFocus == null)
	    bgDecorationNoFocus = new Color(decorations.getDisplay(), new RGB(225, 225, 225));
