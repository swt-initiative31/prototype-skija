...
public class Table extends Composite implements ICustomWidget {
	...
	int defaultBackground() {
		...
		return getDisplay().getSystemColor(SWT.COLOR_WHITE).handle;
	}
	...
	boolean _checkGrow(int count) {
		...
		// int length = small ? items.length + 4 : Math.max (4, items.length * 3
		// /
		// 2);
		...
		// boolean small = getDrawing () && OS.IsWindowVisible (handle);
		// int length = small ? keys.length + 4 : Math.max (4, keys.length * 3 /
		// 2);
	...
	void _checkShrink() {
		...
		//
		// /*
		// * Bug in Windows. Call to OS.LVM_GETITEMCOUNT unexpectedly returns
		// zero,
		// * leading to a possible "ArrayIndexOutOfBoundsException: 4" in SWT
		// table.
		// * So, double check for any existing living items in the table and
		// fixing
	...
	TableItem _getItem(int index, boolean create, int count) {
		...
		// }
		// //TODO - _checkGrow() doesn't return a value, check keys == null
		// instead
		// if (_checkGrow (count)) {
		// if (items [index] != null) return items [index];
		// return items [index] = new TableItem (this, SWT.NONE, -1, false);
		...
		// System.arraycopy(keys, keyIndex, keys, keyIndex + 1, keyCount -
		// keyIndex);
		// System.arraycopy(items, keyIndex, items, keyIndex + 1, keyCount -
		// keyIndex);
		// }
		// keyCount++;
		// keys [keyIndex] = index;
	...
	// if (OS.SendMessage (handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED) ==
	// nmcd.dwItemSpec) {
	...
	// OS.SendMessage (handle, OS. LVM_GETSUBITEMRECT, nmcd.dwItemSpec,
	// itemRect);
	// }
	...
	// }
	// if (explorerTheme && !ignoreCustomDraw) {
	// hotIndex = (nmcd.uItemState & OS.CDIS_HOT) != 0 ? (int)nmcd.dwItemSpec :
	// -1;
	// if (hooks (SWT.EraseItem) && nmcd.left != nmcd.right) {
	// OS.SaveDC (nmcd.hdc);
	...
	// */
	// hwndToolTip = OS.SendMessage (handle, OS.LVM_SETTOOLTIPS, hwndToolTip,
	// hwndToolTip);
	// }
	// }
	// }
	...
	// if (enabled && (int)OS.SendMessage (handle, OS.LVM_GETBKCOLOR, 0, 0) ==
	// OS.CLR_NONE || !enabled && hasCustomBackground()) {
	...
	// long hwndHeader = OS.SendMessage (handle, OS.LVM_GETHEADER, 0, 0);
	// if (OS.SendMessage (hwndHeader, OS.HDM_GETITEMRECT, index, headerRect) !=
	// 0)
	// {
	...
	// }
	//
	// LRESULT CDDS_SUBITEMPOSTPAINT (NMLVCUSTOMDRAW nmcd, long wParam, long
	// lParam)
	// {
	// if (ignoreCustomDraw) return null;
	...
	// int oldColumn = (int)OS.SendMessage (handle, OS.LVM_GETSELECTEDCOLUMN, 0,
	// 0);
	// if (oldColumn == -1) {
	...
	// }
	//
	// LRESULT CDDS_SUBITEMPREPAINT (NMLVCUSTOMDRAW nmcd, long wParam, long
	// lParam)
	// {
	// long hDC = nmcd.hdc;
	...
	// ignoreDrawForeground = ignoreDrawSelection = ignoreDrawFocus =
	...
	// }
	// if (ignoreDrawForeground || hooks (SWT.PaintItem)) code |=
	// OS.CDRF_NOTIFYPOSTPAINT;
	// }
	// int clrText = item.cellForeground != null ? item.cellForeground
	// [nmcd.iSubItem] : -1;
	// if (clrText == -1) clrText = item.foreground;
	// int clrTextBk = item.cellBackground != null ? item.cellBackground
	// [nmcd.iSubItem] : -1;
	...
	// if (!explorerTheme && !ignoreDrawSelection && (style &
	// SWT.FULL_SELECTION) !=
	// 0) {
	...
	// }
	// if (clrSelection != -1) {
	// RECT rect = item.getBounds ((int)nmcd.dwItemSpec, nmcd.iSubItem, true,
	// nmcd.iSubItem != 0, true, false, hDC);
	...
	public void clear(int index) {
		...
		// item.cached = false;
		// }
		// if (currentItem == null && getDrawing () && OS.IsWindowVisible
		// (handle))
		// {
	...
	Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
		...
		boolean widthMustBeComputed = false;
		boolean heightMustBeComputed = false;

		if (wHint != SWT.DEFAULT)
			size.x = wHint;
		else
			widthMustBeComputed = true;

		if (hHint != SWT.DEFAULT) {
			size.y = hHint;
		} else
			heightMustBeComputed = true;

		Point s = null;
		if (widthMustBeComputed || heightMustBeComputed)
			s = calculateSize(widthMustBeComputed, heightMustBeComputed);

		if (widthMustBeComputed)
			size.x = s.x;
		if (heightMustBeComputed)
			size.y = s.y;
	...
	void createHandle() {
		...
		// if ((style & SWT.FULL_SELECTION) != 0) bits1 |=
		// OS.LVS_EX_FULLROWSELECT;
	...
	void createHeaderToolTips() {
		...
		// * a large value.
		// */
		// OS.SendMessage (headerToolTipHandle, OS.TTM_SETMAXTIPWIDTH, 0,
		// 0x7FFF);
	}
	...
	void createItem(TableColumn column, int index) {
		...
		// System.arraycopy (strings, index, temp, index + 1, columnCount -
		// index);
		// item.strings = temp;
		// }
		...
		// System.arraycopy (images, index, temp, index + 1, columnCount -
		// index);
		// item.images = temp;
		// }
		// if (index == 0) {
		...
		// System.arraycopy (cellBackground, index, temp, index + 1, columnCount
		// -
		// index);
		// temp [index] = -1;
		...
		// System.arraycopy (cellForeground, index, temp, index + 1, columnCount
		// -
		// index);
		// temp [index] = -1;
		...
		// System.arraycopy (cellFont, index, temp, index + 1, columnCount -
		// index);
		// item.cellFont = temp;
		// }
		// }
		...
		// * or LVM_SETCOLUMN.
		// */
		// System.arraycopy (columns, index, columns, index + 1, columnCount++ -
		// index);
		// columns [index] = column;
		//
		// /*
		...
		// if ((column.style & SWT.CENTER) == SWT.CENTER) fmt =
		// OS.LVCFMT_CENTER;
		...
		// RECT rect = new RECT ();
		// if (OS.SendMessage (hwndHeader, OS.HDM_GETITEMRECT, index, rect) !=
		// 0) {
...
//	}
	...
	void createItem(TableItem item, int index) {
		...
		if (items == null)
			items = new TableItem[index + 1];
		else if (items.length <= index) {

			TableItem[] ti = new TableItem[index + 1];
			System.arraycopy(items, 0, ti, 0, items.length);
			items = ti;
		...
		if (index == 0) {
			...
			Rectangle before = items[index - 1].getBounds();
			item.setLocation(new Point(1, before.y + before.height + 2)); // +2
																			// for:
																			// one
																			// by
			// other location
		...
		// int result = (int)OS.SendMessage (handle, OS.LVM_INSERTITEM, 0,
		// lvItem);
	...
	private int getTableItemHeight() {
		...
		if (items == null || items.length == 0)
			...
			return items[0].getBounds().height;
	...
	void destroyItem(TableColumn column) {
		...
		// System.arraycopy (strings, 0, temp, 0, index);
		// System.arraycopy (strings, index + 1, temp, index, columnCount -
		// index);
		// item.strings = temp;
		// } else {
		...
		// System.arraycopy (images, 0, temp, 0, index);
		// System.arraycopy (images, index + 1, temp, index, columnCount -
		// index);
		// item.images = temp;
		// } else {
		...
		// int [] temp = new int [columnCount];
		// System.arraycopy (cellForeground, 0, temp, 0, index);
		// System.arraycopy (cellForeground, index + 1, temp, index, columnCount
		// -
		// index);
		// item.cellForeground = temp;
		// }
		...
		// OS.SendMessage (handle, OS.LVM_SETCOLUMNORDERARRAY, newOrder.length,
		// newOrder);
		// /*
	...
	// lvItem.iItem = (int)nmcd.dwItemSpec;
	// long result = OS.SendMessage (handle, OS.LVM_GETITEM, 0, lvItem);
	// boolean selected = (result != 0 && (lvItem.state & OS.LVIS_SELECTED) !=
	// 0);
	...
	// if (selected && (nmcd.iSubItem == 0 || (style & SWT.FULL_SELECTION) !=
	// 0)) {
	// if (OS.GetFocus () == handle || display.getHighContrast ()) {
	// drawSelected = true;
	...
	// cellRect.top, cellRect.right - cellRect.left, cellRect.bottom -
	// cellRect.top), getZoom());
	...
	// } else {
	// ignoreDrawForeground = ignoreDrawBackground = ignoreDrawSelection =
	// ignoreDrawFocus = ignoreDrawHot = true;
	// }
	// if (drawSelected) {
	...
	// true,
	// false, fullText, false, hDC);
	// if ((style & SWT.FULL_SELECTION) == 0) {
	// if (measureEvent != null) {
	// Rectangle boundsInPixels = DPIUtil.scaleUp(measureEvent.getBounds(),
	// getZoom());
	// textRect.right = Math.min (cellRect.right, boundsInPixels.x +
	...
	// if ((style & SWT.FULL_SELECTION) != 0) {
	// int count = (int)OS.SendMessage (hwndHeader, OS.HDM_GETITEMCOUNT, 0, 0);
	// int index = (int)OS.SendMessage (hwndHeader, OS.HDM_ORDERTOINDEX, count -
	// 1,
	// 0);
	...
	// if (OS.GetFocus () != handle && selected && !drawHot) iStateId =
	// OS.LISS_SELECTEDNOTFOCUS;
	// if (drawDrophilited) iStateId = OS.LISS_SELECTED;
	// OS.DrawThemeBackground (hTheme, hDC, OS.LVP_LISTITEM, iStateId, rect,
	// pClipRect);
	// OS.CloseThemeData (hTheme);
	// }
	// } else {
	// if (!ignoreDrawSelection && clrSelectionBk != -1) fillBackground (hDC,
	// clrSelectionBk, textRect);
	// }
	// }
	...
	// }
	//
	// Event sendEraseItemEvent (TableItem item, NMTTCUSTOMDRAW nmcd, int
	// column,
	// RECT cellRect) {
	...
	// }
	//
	// Event sendMeasureItemEvent (TableItem item, int row, int column, long
	// hDC) {
	// GCData data = new GCData ();
	// data.device = display;
	// data.font = item.getFont (column);
	...
	// if (boundsInPixels.x + boundsInPixels.width > width) setScrollWidth
	...
	// */
	// if ((style & SWT.SINGLE) != 0 || hooks (SWT.MouseDown) || hooks
	// (SWT.MouseUp)) {
	// if (pinfo.iItem == -1) {
	// if (!display.captureChanged && !isDisposed ()) {
	// if (OS.GetCapture () != handle) OS.SetCapture (handle);
	...
	// */
	// boolean dragDetect = (state & DRAG_DETECT) != 0 && hooks
	// (SWT.DragDetect);
	// if (!dragDetect) {
	// int flags = OS.LVHT_ONITEMICON | OS.LVHT_ONITEMLABEL;
	...
	// }
	//
	// Event sendPaintItemEvent (TableItem item, NMTTCUSTOMDRAW nmcd, int
	// column,
	// RECT itemRect) {
	// int nSavedDC = OS.SaveDC (nmcd.hdc);
	...
	public void setColumnOrder(int[] order) {
		...
		// int index = order [i];
		// if (index < 0 || index >= columnCount) error
		// (SWT.ERROR_INVALID_RANGE);
	...
	void setDeferResize(boolean defer) {
		...
		// * turned off.
		// */
		// if (hooks (SWT.MeasureItem) || hooks (SWT.EraseItem) || hooks
		// (SWT.PaintItem)) {
	...
	void setCheckboxImageList(int width, int height, boolean fixScroll) {
		...
		// long hBitmap = OS.CreateCompatibleBitmap (hDC, width * count,
		// height);
		...
		// */
		// SIZE size = new SIZE();
		// OS.GetThemePartSize(display.hButtonTheme(), memDC, OS.BP_CHECKBOX, 0,
		// null, OS.TS_TRUE, size);
	...
	public void setRedraw(boolean redraw) {
		...
		// if (hwndHeader != 0) OS.SendMessage (hwndHeader, OS.WM_SETREDRAW, 1,
		// 0);
		// if ((state & HIDDEN) != 0) {
		// state &= ~HIDDEN;
		...
		// if (hwndHeader != 0) OS.SendMessage (hwndHeader, OS.WM_SETREDRAW, 0,
		// 0);
		//
		// /*
		// * When many items are added to a table, it is faster to
	...
	boolean setScrollWidth(TableItem item, boolean force) {
		...
		if (columnCount == 0) {
			...
			// int itemCount = (int)OS.SendMessage (handle, OS.LVM_GETITEMCOUNT,
			// 0,
			// 0);
			// while (index < itemCount) {
			// String string = null;
	...
	public void showColumn(TableColumn column) {
		...
		// } else {
		// int width = Math.min (rect.right - rect.left, itemRect.right -
		// itemRect.left);
	...
	void showItem(int index) {
		...
		// */
		// long counterPage = OS.SendMessage (handle, OS.LVM_GETCOUNTPERPAGE, 0,
		// 0);
		// if (counterPage <= 0) {
		// /*
		// * Bug in Windows. For some reason, LVM_ENSUREVISIBLE can
	...
	public void showSelection() {
		...
		// 450391.
		// */
		// if (display.getActiveShell() == getShell() && (style & SWT.NO_SCROLL)
		// ==
		// 0
	...
	void updateOrientation() {
		...
		// OS.SWP_NOMOVE |
		// OS.SWP_NOZORDER);
		// OS.SetWindowPos (handle, 0, 0, 0, width, height, OS.SWP_NOMOVE |
		// OS.SWP_NOZORDER);
		// }
	...
	boolean updateTextDirection(int textDirection) {
		...
		// if (textDirection == AUTO_TEXT_DIRECTION || (state &
		// HAS_AUTO_DIRECTION)
		// != 0) {
		// for (TableItem item : items) {
		// if (item != null) {
		// item.updateTextDirection(textDirection == AUTO_TEXT_DIRECTION ?
		// AUTO_TEXT_DIRECTION : style & SWT.FLIP_TEXT_DIRECTION);
		// }
		// }
		// }
		// OS.InvalidateRect (handle, null, true);
	...
	// int selection = (int)OS.SendMessage (handle, OS.LVM_GETNEXTITEM, topIndex
	// -
	// 1, OS.LVNI_SELECTED);
	...
	// long rectRgn = OS.CreateRectRgn (rect.left, itemRect.top, rect.right,
	// itemRect.bottom);
	...
	// long memDib = OS.CreateDIBSection (0, bmi, OS.DIB_RGB_COLORS, pBits, 0,
	// 0);
	...
	// if ((style & SWT.CHECK) != 0) {
	// int index = -1;
	// while ((index = (int)OS.SendMessage (handle, OS.LVM_GETNEXTITEM, index,
	// OS.LVNI_SELECTED)) != -1) {
	...
	// return LRESULT.ZERO;
	// }
	// if (pinfo.iItem != -1) callWindowProc (handle, OS.WM_LBUTTONDBLCLK,
	// wParam,
	// lParam);
	// if (!display.captureChanged && !isDisposed ()) {
	// if (OS.GetCapture () != handle) OS.SetCapture (handle);
	...
	// if (pinfo.iItem != -1) callWindowProc (handle, OS.WM_RBUTTONDBLCLK,
	// wParam,
	// lParam);
	// }
	// if (!display.captureChanged && !isDisposed ()) {
	...
	// int rowCount = (int)OS.SendMessage (handle, OS.LVM_GETCOUNTPERPAGE, 0,
	// 0);
	// if (rowCount > V_SCROLL_LIMIT) fixScroll = getDrawing () &&
	...
	// if (ignoreShrink) {
	// /*
	// * Feature in Windows Vista and newer. Using LVM_REDRAWITEMS causes
	// LVN_GETDISPINFO
	...
	// rect.left = OS.LVIR_BOUNDS;
	// ignoreCustomDraw = true;
	// long code = OS.SendMessage (handle, OS. LVM_GETITEMRECT, plvfi.iItem,
	// rect);
	// ignoreCustomDraw = false;
	...
	// } else {
	// String [] strings = item.strings;
	// if (strings != null && plvfi.iSubItem < strings.length) string = strings
	// [plvfi.iSubItem];
	// }
	// if (string != null) {
	// /*
	...
	// * is set.
	// */
	// int length = Math.min (string.length (), Math.max (0, plvfi.cchTextMax -
	// 1));
	...
	// } else {
	// Image [] images = item.images;
	// if (images != null && plvfi.iSubItem < images.length) image = images
	// [plvfi.iSubItem];
	// }
	// if (image != null) {
	...
	// case OS.CDDS_ITEMPREPAINT: return CDDS_ITEMPREPAINT (nmcd, wParam,
	// lParam);
	// case OS.CDDS_ITEMPOSTPAINT: return CDDS_ITEMPOSTPAINT (nmcd, wParam,
	// lParam);
	// case OS.CDDS_SUBITEMPREPAINT: return CDDS_SUBITEMPREPAINT (nmcd, wParam,
	// lParam);
	// case OS.CDDS_SUBITEMPOSTPAINT: return CDDS_SUBITEMPOSTPAINT (nmcd,
	// wParam,
	// lParam);
	...
	// /* Drawing here will be deleted by further drawing steps, even with
	// OS.CDRF_SKIPDEFAULT.
	// Changing the TextColor and returning OS.CDRF_NEWFONT has no effect. */
	// return new LRESULT (customHeaderDrawing() ? OS.CDRF_NOTIFYITEMDRAW |
	// OS.CDRF_NOTIFYPOSTPAINT : OS.CDRF_DODEFAULT);
	// }
	...
	// }
	// /*
	// * Don't change the header background color for set selected column,
	// similar
	// to
	// * Windows 10 which itself does not use any different color for sort
	// header.
	// For
	// * more details refer bug 536020
	// */
	//// else if (columns[(int) nmcd.dwItemSpec] == sortColumn && sortDirection
	// !=
	// SWT.NONE) {
	...
	//
	// return new LRESULT(OS.CDRF_SKIPDEFAULT); // if we got here, we will paint
	// everything ourself
	// }
	...
	// /*
	// * Sort indicator size needs to scale as per the Native Windows OS DPI
	// level
	...
	//
	// /* Windows 7 and 10 always draw a nearly invisible vertical line between
	// the
	// columns, even if lines are disabled.
	// This line uses no fixed color constant, but calculates it from the
	// background
	// color.
	// The method getSlightlyDifferentColor gives us a color, that is near
	// enough to
	// the windows algorithm.
	//
	// NOTE: This code has no effect since Bug 517003, because next
	// OS.Polyline()
	// draws over the same coords.
	//
	// long pen = OS.CreatePen (OS.PS_SOLID, getGridLineWidthInPixels(),
	// getSlightlyDifferentColor(getHeaderBackgroundPixel()));
	// long oldPen = OS.SelectObject (nmcd.hdc, pen);
	// OS.Polyline(nmcd.hdc, new int[] {rects[i].right-alignmentCorrection,
	// rects[i].top, rects[i].right-alignmentCorrection, rects[i].bottom}, 2);
	// OS.SelectObject (nmcd.hdc, oldPen);
	// OS.DeleteObject (pen);
	// */
	//
	// int lineColor = (display.tableHeaderLinePixel != -1) ?
	// display.tableHeaderLinePixel : OS.GetSysColor(OS.COLOR_3DFACE);
	// long pen = OS.CreatePen (OS.PS_SOLID, getGridLineWidthInPixels(),
	// lineColor);
	// long oldPen = OS.SelectObject (nmcd.hdc, pen);
	...
	// OS.Polyline(nmcd.hdc, new int[] {rects[i].right - alignmentCorrection,
	// rects[i].top, rects[i].right - alignmentCorrection, rects[i].bottom}, 2);
	// /* To differentiate header & content area, always draw the line separator
	// between header & first row. */
	// if (i == 0) OS.Polyline(nmcd.hdc, new int[] {nmcd.left, nmcd.bottom-1,
	// nmcd.right+1, nmcd.bottom-1}, 2);
	// OS.SelectObject (nmcd.hdc, oldPen);
	// OS.DeleteObject (pen);
	//
	...
	// data.device = display;
	// GC gc = createNewGC(nmcd.hdc, data);
	// int y = Math.max (0, (nmcd.bottom -
	// columns[i].image.getBoundsInPixels().height) / 2);
	// int zoom = getZoom();
	// gc.drawImage (columns[i].image, DPIUtil.scaleDown(x, zoom),
	...
	// }
	//
	// // always draw the highlighted border at the end, to avoid overdrawing by
	// other borders.
	// if (highlightedHeaderDividerX != -1) {
	// long pen = OS.CreatePen (OS.PS_SOLID, 4,
	// OS.GetSysColor(OS.COLOR_HIGHLIGHT));
	// long oldPen = OS.SelectObject (nmcd.hdc, pen);
	// OS.Polyline(nmcd.hdc, new int[] {highlightedHeaderDividerX, nmcd.top,
	// highlightedHeaderDividerX, nmcd.bottom}, 2);
	// OS.SelectObject (nmcd.hdc, oldPen);
	// OS.DeleteObject (pen);
	// }
	//
	...
	// if (hFont != -1) hFont = OS.SelectObject (hDC, hFont);
	// Event event = sendMeasureItemEvent (item, pinfo.iItem, pinfo.iSubItem,
	// hDC);
	// if (!isDisposed () && !item.isDisposed ()) {
	// RECT itemRect = new RECT ();
	// Rectangle boundsInPixels = DPIUtil.scaleUp(event.getBounds(), getZoom());
	// OS.SetRect (itemRect, boundsInPixels.x, boundsInPixels.y,
	// boundsInPixels.x +
	// boundsInPixels.width, boundsInPixels.y + boundsInPixels.height);
	...
	// toolRect.top;
	// OS.SetWindowPos (hwndToolTip, 0, toolRect.left , toolRect.top, width,
	// height,
	// flags);
	// } else {
	...
	// }
	// RECT cellRect = item.getBounds (pinfo.iItem, pinfo.iSubItem, true, true,
	// true, true, hDC);
	// RECT clientRect = new RECT ();
	// OS.GetClientRect (handle, clientRect);
	...
	// RECT imageRect = item.getBounds (pinfo.iItem, pinfo.iSubItem, false,
	// true,
	// false, false, hDC);
	// Point size = imageList == null ? new Point (rect.width, rect.height) :
	// imageList.getImageSize ();
	// int y = imageRect.top + Math.max (0, (imageRect.bottom - imageRect.top -
	// size.y) / 2);
	// int zoom = getZoom();
	// rect = DPIUtil.scaleDown(rect, zoom);
	// gc.drawImage (image, rect.x, rect.y, rect.width, rect.height,
	// DPIUtil.scaleDown(x, zoom), DPIUtil.scaleDown(y, zoom),
	// DPIUtil.scaleDown(size.x, zoom), DPIUtil.scaleDown(size.y, zoom));
	...
	// }
	// if (hooks (SWT.PaintItem)) {
	// RECT itemRect = item.getBounds (pinfo.iItem, pinfo.iSubItem, true, true,
	// false, false, hDC);
	...
	// }
	//
	// private static void handleDPIChange(Widget widget, int newZoom, float
	// scalingFactor) {
	...
	// scrollWidth = Math.round(OS.SendMessage (table.handle,
