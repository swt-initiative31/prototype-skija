...
public class MenuItem extends Item {
	...
	// There is a weird behavior in the Windows API with menus in OWENERDRAW mode
	// that the returned
	// value in wmMeasureChild is increased by a fixed value (in points) when
	// wmDrawChild is called
	// This static is used to mitigate this increase
	private final static int WINDOWS_OVERHEAD = 6;

	/**
	 * Constructs a new instance of this class given its parent (which must be a
	 * <code>Menu</code>) and a style value describing its behavior and appearance.
	...
	/**
	 * Constructs a new instance of this class given its parent (which must be a
	 * <code>Menu</code>), a style value describing its behavior and appearance, and
	 ...
	 * the arm events are generated for the control, by sending it one of the
	 * messages defined in the <code>ArmListener</code> interface.
	 *
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * the help events are generated for the control, by sending it one of the
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * the menu item is selected by the user, by sending it one of the messages
	 * defined in the <code>SelectionListener</code> interface.
	 * <p>
	 * When <code>widgetSelected</code> is called, the stateMask field of the event
	 ...
	 * </p>
	 *
	 * @param listener the listener which should be notified when the menu item is
...
//}
	...
	/**
	 * Returns the widget accelerator. An accelerator is the bit-wise OR of zero or
	 * more modifier masks and a key. Examples:
	 * <code>SWT.CONTROL | SWT.SHIFT | 'T', SWT.ALT | SWT.F2</code>. The default
	 * value is zero, indicating that the menu item does not have an accelerator.
	 *
...
//	}
	 ...
	 * otherwise. A disabled menu item is typically not selectable from the user
	 ...
	 * disabled menu item is typically not selectable from the user interface and
...
//	accelerator = 0;
	 ...
	 * when the arm events are generated for the control.
	 *
	...
	public void setEnabled(boolean enabled) {
		...
		if (this.enabled == enabled)
			return;
		/*
		 * Feature in Windows. For some reason, when the menu item is a separator,
		 * GetMenuItemInfo() always indicates that the item is not enabled. The fix is
		 * to track the enabled state for separators.
		 */
		if ((style & SWT.SEPARATOR) != 0) {
			if (enabled) {
				state &= ~DISABLED;
	 ...
	 *
	 * @param id the new identifier. This must be a non-negative value.
	 ...
	 * <p>
	 * Note: This operation is a <em>HINT</em> and is not supported on platforms
	 * that do not have this concept (for example, Windows NT). Some platforms (such
	 * as GTK3) support images alongside check boxes.
	 * </p>
	 *
	...
	/**
	 * Sets the receiver's pull down menu to the argument. Only <code>CASCADE</code>
	 * menu items can have a pull down menu. The sequence of key strokes, button
	 * presses and/or button releases that are used to request a pull down menu is
	 * platform specific.
	 * <p>
	 * Note: Disposing of a menu item that has a pull down menu will dispose of the
	 * menu. To avoid this behavior, set the menu to null before the menu item is
	 * disposed.
	 * </p>
	 *
	 ...
	 * @exception IllegalArgumentException
	 *                                     <ul>
	 *                                     <li>ERROR_MENU_NOT_DROP_DOWN - if the
	 *                                     menu is not a drop down menu</li>
	 *                                     <li>ERROR_MENUITEM_NOT_CASCADE - if the
	 *                                     menu item is not a
...
//	if (menu != null) menu._setOrientation (orientation);
	...
	/**
	 * Sets the receiver's text. The string may include the mnemonic character and
	 * accelerator text.
	 * <p>
	 ...
	 * displayed to the user, typically indicating the key stroke that will cause
	 * the item to become selected. On most platforms, the accelerator text appears
	 * right aligned in the menu. Setting the accelerator text does not install the
	 * accelerator key sequence. The accelerator key sequence is installed using
	 * #setAccelerator.
	 * </p>
	 *
	 ...
	 * menu item that has a default tool tip, setting the tool tip text to an empty
	 * string replaces the default, causing no tool tip text to be shown.
	 * <p>
	 ...
	 * {@link Shell#setMenuBar(Menu) shell menubar} on Windows, Mac, and Ubuntu
	 * Unity desktop.
	 * </p>
	 * <p>
