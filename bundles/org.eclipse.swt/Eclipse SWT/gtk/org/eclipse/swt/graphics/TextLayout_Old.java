...
public final class TextLayout_Old extends Resource {
	...
	public FontMetrics getFixedLineMetrics(Device device) {
	    ...
	    result.averageCharWidthInPoints = DPIUtil.autoScaleDown(device, nfm.averageCharWidthInPoints);
	...
	public long gdk_pango_layout_get_clip_region(long layout, int x_origin, int y_origin, int[] index_ranges,
		int n_ranges) {
	    ...
	    long rgn = GDK.gdk_pango_layout_get_clip_region(layout, x_origin, y_origin + yAdjustInPixels, index_ranges,
		    n_ranges);
	...
	public void pango_layout_iter_get_line_extents(long iter, PangoRectangle ink_rect,
...
public TextLayout_Old(Device device) {
	...
	if (stylesCount == 2 && styles[0].style == null && ascentInPoints == -1 && descentInPoints == -1
		&& segments == null) {
	...
	boolean useMinAscentDescent = !metricsAdapter.isFixedMetrics()
		&& (ascentInPoints != -1 || descentInPoints != -1);
	if (useMinAscentDescent && segementsLength > 0) {
	    PangoRectangle rect = new PangoRectangle();
	    if (ascentInPoints != -1)
		rect.y = -(DPIUtil.autoScaleUp(getDevice(), ascentInPoints) * OS.PANGO_SCALE);
	    rect.height = DPIUtil.autoScaleUp(getDevice(), (Math.max(0, ascentInPoints) + Math.max(0, descentInPoints)))
		    * OS.PANGO_SCALE;
	...
	for (int i = 0; i < stylesCount - 1; i++) {
		...
		if (style.underlineColor != null) {
		    ...
		    attr = OS.pango_attr_underline_color_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF),
			    (short) (rgba.blue * 0xFFFF));
		    if (attr != 0) {
			OS.memmove(attribute, attr, PangoAttribute.sizeof);
			attribute.start_index = byteStart;
			attribute.end_index = byteEnd;
		...
		attr = OS.pango_attr_foreground_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF),
			(short) (rgba.blue * 0xFFFF));
		OS.memmove(attribute, attr, PangoAttribute.sizeof);
		attribute.start_index = byteStart;
		attribute.end_index = byteEnd;
    ...
    void drawWithCairo(GC gc, int x, int y, int start, int end, int yExtent, boolean fullSelection, GdkRGBA fg,
	    GdkRGBA bg) {
	...
	for (int i = 0; i < stylesCount - 1; i++) {
		...
		if (rgn != 0) {
			...
			for (int j = 0; j < cairoDashes.length; j++) {
			    cairoDashes[j] = width == 0 || data.lineStyle == SWT.LINE_CUSTOM ? dashes[j]
				    : dashes[j] * width;
			}
			...
			Cairo.memmove(rect, rects[0] + (j * cairo_rectangle_int_t.sizeof),
	 ...
	 * includes areas from lines outside of the requested range. The fix is to
	 * subtract these areas from the clip region.
	 */
	...
	/*
	 * Feature in GTK. pango_layout_xy_to_index() returns the logical end/start
	 * offset of a line when the coordinates are outside the line bounds. In SWT the
	 * correct behavior is to return the closest visual offset. The fix is to clamp
	 * the coordinates inside the line bounds.
	 */
	long iter = OS.pango_layout_get_iter(layout);
	if (iter == 0)
	    SWT.error(SWT.ERROR_NO_HANDLES);
	...
	if (trailing != null) {
	    ...
	    if (piTrailing[0] != 0) {
		trailing[0] = (int) OS.g_utf8_offset_to_utf16_offset(ptr,
			OS.g_utf8_pointer_to_offset(ptr, ptr + index[0]) + piTrailing[0]) - offset;
	    }
	}
	...
	/*
	 * Bug in Pango. Pango 1.2.2 will cause a segmentation fault if a style is not
	 * applied for a whole ligature. The fix is to applied the style for the whole
	 * ligature.
	 *
	...
	/*
	 * Bug in Pango. A change in the tab stop array is not automatically reflected
	 * in the pango layout object because the call pango_layout_set_tabs() does not
	 * free the lines cache. The fix to use pango_layout_context_changed() to free
	 * the lines cache.
	 */
	OS.pango_layout_context_changed(layout);
    }
