...
void deregister () {
	if (handle == 0) return;
	if (((getWidget().state) & HANDLE) != 0)
		getWidget().display.removeWidget(handle);
}

void destroyWidget () {
	long topHandle = topHandle ();
	releaseHandle ();
	if (topHandle != 0 && (getWidget().state & HANDLE) != 0) {
		if (GTK.GTK4) {
...
long dpiChanged (long object, long arg0) {
	...
	if (oldScaleFactor != newScaleFactor) {
		getWidget().display.dpiChanged(newScaleFactor);
		...
		event.doit = true;
		getWidget().notifyListeners(SWT.ZoomChanged, event);
	}
...
void hookEvents () {
	if (handle != 0) {
		OS.g_signal_connect(handle, OS.dpi_changed, getWidget().display.notifyProc, Widget.DPI_CHANGED);
	}
}
...
void register () {
	if (handle == 0) return;
	if ((getWidget().state & HANDLE) != 0)
		getWidget().display.addWidget(handle, getWidget());
}
...
boolean sendKeyEvent (int type, long event) {
	...
	if (string == 0 || OS.g_utf16_strlen (string, length) <= 1) {
		...
		getWidget().sendEvent(type, javaEvent);
...
void sendSelectionEvent (int eventType, Event event, boolean send) {
	...
	getWidget().sendEvent(eventType, event, send);
}
...
public void setData (String key, Object value) {
	...
	if (key == null)
		getWidget().error(SWT.ERROR_NULL_ARGUMENT);

	if (key.equals (KEY_CHECK_SUBWINDOW)) {
		if (value != null && value instanceof Boolean) {
			if (((Boolean)value).booleanValue ()) {
				getWidget().state |= CHECK_SUBWINDOW;
			} else {
				getWidget().state &= ~CHECK_SUBWINDOW;
			}
		}
	...
	switch (eventKeyval[0]) {
		...
		default: {
			if (javaEvent.keyCode == 0) {
				...
				Map<Integer, Integer> groupLatinKeysCount = getWidget().display.getGroupKeysCount();
				if (!groupLatinKeysCount.containsKey(group)) {
					group = getWidget().display.getLatinKeyGroup();
				}
...
public String toString () {
	...
	if (!getWidget().isDisposed()) {
		...
		if (getWidget().isValidThread())
			string = getWidget().getNameText();
	}
	return getWidget().getName() + " {" + string + "}";
}
...
void setToolTipText(long tipWidget, String string) {
	...
	if (string != null && !string.isEmpty()) {
		char[] chars = getWidget().fixMnemonic(string, false, true);
