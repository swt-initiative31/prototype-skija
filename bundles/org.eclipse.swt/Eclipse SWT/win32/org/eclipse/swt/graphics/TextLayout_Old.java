...
public final class TextLayout_Old extends Resource {
     ...
     * will fail to render in ScriptTextOut, ScriptShape, ScriptPlace so such long
     * runs need to be split into shorter runs. Because it is expensive to keep
     * testing (with Script*) to maximize the length we use this heuristic to
     * minimize the length. However splitting the runs into too short pieces affects
     * performance, so this is a balance.
     */
    static final int MAX_RUN_LENGTH = 32000;
    /**
     * When splitting a run (see {@link #MAX_RUN_LENGTH}) the run needs to split in
     * a way that does not affect the display of the glyphs, so it is important to
     * not split the run in the middle of a glyph. We use the same info to find
     * where we can wrap text to find where we can break the runs (ScriptBreak's
     * info). This setting limits how far back from {@link #MAX_RUN_LENGTH} the code
	...
	public void setFixedLineMetrics(FontMetrics metrics) {
	    ...
	    result.tmAscent = ((NativeFontMetrics) metrics.innerFontMetrics).handle.tmAscent;
	    result.tmDescent = ((NativeFontMetrics) metrics.innerFontMetrics).handle.tmDescent;
	    result.tmHeight = ((NativeFontMetrics) metrics.innerFontMetrics).handle.tmHeight;
	    result.tmInternalLeading = ((NativeFontMetrics) metrics.innerFontMetrics).handle.tmInternalLeading;
	...
	public int ScriptTextOut(long hdc, long psc, int x, int y, int fuOptions, RECT lprc, SCRIPT_ANALYSIS psa,
		long pwcReserved, int iReserved, long pwGlyphs, int cGlyphs, long piAdvance, long piJustify,
		long pGoffset) {
	    if (isFixedMetrics()) {
	    ...
	    return OS.ScriptTextOut(hdc, psc, x, y, fuOptions, lprc, psa, pwcReserved, iReserved, pwGlyphs, cGlyphs,
...
public TextLayout_Old(Device device) {
	...
	if (runs != null)
	    return;
	long hDC = gc != null ? ((NativeGC) gc.innerGC).handle : device.internal_new_GC(null);
	...
	for (int i = 0; i < allRuns.length - 1; i++) {
		...
		if (j == tabsLength) {
		    ...
		    int lastTabWidth = tabsLength > 1 ? tabsInPixels[tabsLength - 1] - tabsInPixels[tabsLength - 2]
			    : tabsInPixels[0];
		    if (lastTabWidth > 0) {
		...
		/*
		 * This block adjusts the indentation after merged tabs stops. The extra tabs
		 * are removed in merge.
		 */
		...
		while (i >= lineStart) {
		    ...
		    /*
		     * Bug in Windows. For some reason Uniscribe sets the fSoftBreak flag for the
		     * first letter after a letter with an accent. This cause a break line to be set
		     * in the middle of a word. The fix is to detect the case and ignore fSoftBreak
		     * forcing the algorithm keep searching.
		     */
		...
		if (wrapEntireRun) {
		    ...
		    /*
		     * No soft-break or line-feed found. Avoid breaking a run at the first character
		     * (firstStart == 0) unless it's the only run available (firstIndice ==
     ...
     *                            rectangular area where the text is to be drawn
     * @param selectionStart      the offset where the selections starts, or -1
     *                            indicating no selection
     * @param selectionEnd        the offset where the selections ends, or -1
     *                            indicating no selection
     * @param selectionForeground selection foreground, or NULL to use the system
     *                            default color
     * @param selectionBackground selection background, or NULL to use the system
     *                            default color
     *
     * @exception SWTException
     *                                     <ul>
     ...
     * <code>SWT.DELIMITER_SELECTION</code> or <code>SWT.FULL_SELECTION</code> to
     * specify the selection behavior on all lines except for the last line, and can
     * also include <code>SWT.LAST_LINE_SELECTION</code> to extend the specified
     * selection behavior to the last line.
     * </p>
     *
     * @param gc                  the GC to draw
     * @param x                   the x coordinate of the top left corner of the
	...
	checkLayout();
	drawInPixels(gc, DPIUtil.scaleUp(getDevice(), x, getZoom(gc)), DPIUtil.scaleUp(getDevice(), y, getZoom(gc)),
		selectionStart, selectionEnd, selectionForeground, selectionBackground, flags);
    }

    private int getNativeZoom(GC gc) {
	if (gc != null) {
	    return ((NativeGC) gc.innerGC).data.nativeZoom;
	}
	return nativeZoom;
    }
	...
	if (gdip) {
	    ...
	    gdipForeground = ((NativeGC) gc.innerGC).getFgBrush();
	} else {
	    state = OS.SaveDC(hdc);
	...
	for (int line = 0; line < runs.length; line++) {
	    ...
	    int lineHeight = DPIUtil.scaleUp(getDevice(), lineY[line + 1] - lineY[line] - lineSpacingInPoints,
		    getZoom(gc));
		...
		if (extents) {
		    ...
		    if (gdip) {
			Gdip.Graphics_FillRectangle(gdipGraphics, gdipSelBackground, drawX + lineWidthInPixels[line],
				drawY, width, lineHeightWithSpacing);
		    } else {
			OS.SelectObject(hdc, selBackground);
			OS.PatBlt(hdc, drawX + lineWidthInPixels[line], drawY, width, lineHeightWithSpacing,
				OS.PATCOPY);
		    }
		}
	    }
		...
		baselineInPixels = Math.max(baselineInPixels,
			DPIUtil.scaleUp(getDevice(), run.ascentInPoints, getZoom(gc)));
		...
		if (drawX + run.width >= clip.x) {
			...
			if (gdip) {
				...
				pRect = drawRunTextGDIP(gc, gdipGraphics, run, rect, gdipFont, baselineInPixels, gdipFg,
				...
				int fg = style != null && style.underline && style.underlineStyle == SWT.UNDERLINE_LINK
					? linkColor
					: foreground;
				pRect = drawRunTextGDIPRaster(gc, gdipGraphics, run, rect, baselineInPixels, fg,
					selForeground, selectionStart, selectionEnd);
			    }
			    underlineClip = drawUnderlineGDIP(gc, gdipGraphics, x, drawY + baselineInPixels,
			    ...
			    strikeoutClip = drawStrikeoutGDIP(gc, gdipGraphics, x, drawY + baselineInPixels, lineRuns,
				    i, gdipFg, gdipSelForeground, strikeoutClip, pRect, selectionStart, selectionEnd,
				    alpha, clip);
			    borderClip = drawBorderGDIP(gdipGraphics, x, drawY, lineHeight, lineRuns, i, gdipFg,
				    gdipSelForeground, borderClip, pRect, selectionStart, selectionEnd, alpha, clip);
			} else {
			    int fg = style != null && style.underline && style.underlineStyle == SWT.UNDERLINE_LINK
				    ? linkColor
				    : foreground;
			    pRect = drawRunText(gc, hdc, run, rect, baselineInPixels, fg, selForeground, selectionStart,
				    selectionEnd);
			    underlineClip = drawUnderline(gc, hdc, x, drawY + baselineInPixels, lineUnderlinePos,
				    drawY + lineHeight, lineRuns, i, fg, selForeground, underlineClip, pRect,
				    selectionStart, selectionEnd, clip);
			    strikeoutClip = drawStrikeout(gc, hdc, x, drawY + baselineInPixels, lineRuns, i, fg,
    ...
    RECT drawBorder(long hdc, int x, int y, int lineHeight, StyleItem[] line, int index, int color, int selectionColor,
	    RECT clipRect, RECT pRect, int selectionStart, int selectionEnd, Rectangle drawClip) {
	...
	if (index + 1 >= line.length || lastRunVisible || line[index + 1].lineBreak
    ...
    void drawRunBackgroundGDIP(StyleItem run, long graphics, RECT rect, int selectionStart, int selectionEnd, int alpha,
	    long selBrush, boolean hasSelection) {
	int end = run.start + run.length - 1;
	boolean fullSelection = hasSelection && selectionStart <= run.start && selectionEnd >= end;
	if (fullSelection) {
    ...
    RECT drawRunText(GC gc, long hdc, StyleItem run, RECT rect, int baselineInPixels, int color, int selectionColor,
	...
	metricsAdapter.ScriptTextOut(hdc, run.psc, x, y, 0, null, run.analysis, 0, 0, run.glyphs, run.glyphCount,
		run.advances, run.justify, run.goffsets);
	if (partialSelection) {
	    getPartialSelection(run, selectionStart, selectionEnd, rect);
	    OS.SetTextColor(hdc, selectionColor);
	    metricsAdapter.ScriptTextOut(hdc, run.psc, x, y, OS.ETO_CLIPPED, rect, run.analysis, 0, 0, run.glyphs,
		    run.glyphCount, run.advances, run.justify, run.goffsets);
	}
	return fullSelection || partialSelection ? rect : null;
    }

    RECT drawRunTextGDIP(GC gc, long graphics, StyleItem run, RECT rect, long gdipFont, int baselineInPixels,
	    long color, long selectionColor, int selectionStart, int selectionEnd, int alpha) {
	int end = run.start + run.length - 1;
	boolean hasSelection = selectionStart <= selectionEnd && selectionStart != -1 && selectionEnd != -1;
	boolean fullSelection = hasSelection && selectionStart <= run.start && selectionEnd >= end;
	boolean partialSelection = hasSelection && !fullSelection
	...
	if (partialSelection) {
	    ...
	    Gdip.Graphics_DrawDriverString(graphics, run.glyphs, run.glyphCount, gdipFont, selectionColor, points, 0,
		    0);
	    Gdip.Graphics_Restore(graphics, gstate);
	}
	if (isMirrored) {
    ...
    RECT drawRunTextGDIPRaster(GC gc, long graphics, StyleItem run, RECT rect, int baselineInPixels, int color,
	...
	OS.SetBkMode(hdc, OS.TRANSPARENT);
	RECT pRect = drawRunText(gc, hdc, run, rect, baselineInPixels, color, selectionColor, selectionStart,
		selectionEnd);
	OS.RestoreDC(hdc, state);
    ...
    RECT drawStrikeout(GC gc, long hdc, int x, int baselineInPixels, StyleItem[] line, int index, int color,
	    int selectionColor, RECT clipRect, RECT pRect, int selectionStart, int selectionEnd, Rectangle drawClip) {
	StyleItem run = line[index];
	TextStyle style = run.style;
	if (style == null)
	    return null;
	...
	if (index + 1 >= line.length || lastRunVisible || line[index + 1].lineBreak
		...
		if (clipRect.right == -1)
		    clipRect.right = 0x7ffff;
		OS.SetRect(clipRect, Math.max(rect.left, clipRect.left), rect.top, Math.min(rect.right, clipRect.right),
			rect.bottom);
    ...
    RECT drawStrikeoutGDIP(GC gc, long graphics, int x, int baselineInPixels, StyleItem[] line, int index, long color,
	    long selectionColor, RECT clipRect, RECT pRect, int selectionStart, int selectionEnd, int alpha,
	    Rectangle drawClip) {
	StyleItem run = line[index];
	TextStyle style = run.style;
    ...
    RECT drawUnderline(GC gc, long hdc, int x, int baselineInPixels, int lineUnderlinePos, int lineBottom,
	    StyleItem[] line, int index, int color, int selectionColor, RECT clipRect, RECT pRect, int selectionStart,
	    int selectionEnd, Rectangle drawClip) {
	StyleItem run = line[index];
	TextStyle style = run.style;
	if (style == null)
	    return null;
    ...
    RECT drawUnderlineGDIP(GC gc, long graphics, int x, int baselineInPixels, int lineUnderlinePos, int lineBottom,
	    StyleItem[] line, int index, long color, long selectionColor, RECT clipRect, RECT pRect, int selectionStart,
	    int selectionEnd, int alpha, Rectangle drawClip) {
	StyleItem run = line[index];
	TextStyle style = run.style;
	if (style == null)
	    return null;
	...
	if (index + 1 >= line.length || lastRunVisible || line[index + 1].lineBreak
		...
		if (gdipRect != null) {
		    ...
		    gstate = Gdip.Graphics_Save(graphics);
		    Gdip.Graphics_SetClip(graphics, gdipRect, Gdip.CombineModeIntersect);
		    Gdip.Graphics_FillRectangle(graphics, selectionColor, rect.left, rect.top, rect.right - rect.left,
			    rect.bottom - rect.top);
		    if (style.underlineStyle == SWT.UNDERLINE_DOUBLE) {
			Gdip.Graphics_FillRectangle(graphics, selectionColor, rect.left,
				rect.top + run.underlineThickness * 2, rect.right - rect.left, rect.bottom - rect.top);
		    }
		    Gdip.Graphics_Restore(graphics, gstate);
		}
		break;
    ...
    /**
     * Returns the bounds of the receiver. The width returned is either the width of
     * the longest line or the width set using {@link TextLayout#setWidth(int)}. To
     * obtain the text bounds of a line use {@link TextLayout#getLineBounds(int)}.
     *
    ...
    /**
     * Returns the bounds for the specified range of characters. The bounds is the
     * smallest rectangle that encompasses all characters in the range. The start
     * and end offsets are inclusive and will be clamped if out of range.
     *
	...
	/*
	 * use the high surrogate for the start offset and the low surrogate for the end
	 * offset
	 */
    ...
    /**
     * Returns the embedding level for the specified character offset. The embedding
     * level is usually used to determine the directionality of a character in
     * bidirectional text.
     *
     ...
     *                                     character offset is out of range</li>
     *                                     </ul>
     * @exception SWTException
     *                                     <ul>
     *                                     <li>ERROR_GRAPHIC_DISPOSED - if the
     ...
     * @exception IllegalArgumentException
     *                                     <ul>
     *                                     <li>ERROR_INVALID_ARGUMENT - if the line
	...
	int height = DPIUtil.scaleUp(getDevice(), lineY[lineIndex + 1] - lineY[lineIndex] - lineSpacingInPoints,
		getZoom());
    ...
    /**
     * Returns the line offsets. Each value in the array is the offset for the first
     * character in a line except for the last value, which contains the length of
     * the text.
     *
     ...
     * the leading or trailing edge of the cluster.
     *
     * @param offset   the character offset
	...
	if (offset == length) {
	    return new Point(getLineIndentInPixel(line) + lineWidthInPixels[line],
	...
	while (high - low > 1) {
		...
		if (run.style != null && run.style.metrics != null) {
		    GlyphMetrics metrics = run.style.metrics;
		    width = DPIUtil.scaleUp(getDevice(), metrics.width, getZoom())
		...
		return new Point(run.x + width,
			DPIUtil.scaleUp(getDevice(), lineY[line], getZoom()) + getScaledVerticalIndent());
	    }
	}
     ...
     * character, the trailing argument will be filled in to indicate whether the
     * point is closer to the leading edge (0) or the trailing edge (1). When the
     * point is over a cluster composed of multiple characters, the trailing
     * argument will be filled with the position of the character in the cluster
	...
	checkLayout();
	return getOffsetInPixels(DPIUtil.scaleUp(getDevice(), x, getZoom()), DPIUtil.scaleUp(getDevice(), y, getZoom()),
		trailing);
    }
    ...
    /**
     * Returns the previous offset for the specified offset and movement type. The
     * movement is one of <code>SWT.MOVEMENT_CHAR</code>,
	...
	while (segmentCount < nSegments) {
	    ...
	    char separator = segmentsChars != null && segmentsChars.length > segmentCount ? segmentsChars[segmentCount]
		    : defaultSeparator;
	    newChars[charCount + segmentCount++] = separator;
	}
    ...
    /**
     * Gets the receiver's text, which will be an empty string if it has never been
     * set.
     *
     ...
     * <p>
     * This method gets the dispose state for the text layout. When a text layout
     * has been disposed, it is an error to invoke any other method (except
     * {@link #dispose()}) using the text layout.
     * </p>
     *
	...
	// and https://bugzilla.mozilla.org/show_bug.cgi?id=366643 which was a similar
	// bug
	// in Mozilla. The MSDN docs have been updated since the Mozilla bug to make
	// this clear
...
//	if (hr == E_OUTOFMEMORY) //TODO handle it
	...
	while (start < end) {
		 ...
		 * could be a performance penalty because of too many runs. The tabs need to be
		 * "restored", see computeRuns
		 */
	 ...
	 * make sure that at least we don't split a surrogate pair. This can happen if
	 * ScriptBreak above identifies nowhere that can be split, and the last
	...
	/*
	 * Feature in Windows. If the orientation is RTL Uniscribe will resolve the
	 * level of line breaks to 1, this can cause the line break to be reorder to the
	 * middle of the line. The fix is to set the level to zero to prevent it to be
	 * reordered.
	 */
    ...
    /**
     * Sets the text alignment for the receiver. The alignment controls how a line
     * of text is positioned horizontally. The argument should be one of
    ...
    /**
     * Sets the ascent of the receiver. The ascent is distance in points from the
     * baseline to the top of the line and it is applied to all lines. The default
     * value is <code>-1</code> which means that the ascent is calculated from the
     * line fonts.
     *
     ...
     *                                     <ul>
     *                                     <li>ERROR_INVALID_ARGUMENT - if the
     *                                     ascent is less than <code>-1</code></li>
     *                                     </ul>
     * @exception SWTException
     *                                     <ul>
     *                                     <li>ERROR_GRAPHIC_DISPOSED - if the
    ...
    /**
     * Sets the descent of the receiver. The descent is distance in points from the
     * baseline to the bottom of the line and it is applied to all lines. The
     * default value is <code>-1</code> which means that the descent is calculated
     * from the line fonts.
     *
     ...
     *                                     <ul>
     *                                     <li>ERROR_INVALID_ARGUMENT - if the
     *                                     descent is less than <code>-1</code></li>
     *                                     </ul>
     * @exception SWTException
     *                                     <ul>
     *                                     <li>ERROR_GRAPHIC_DISPOSED - if the
    ...
    /**
     * Forces line heights in receiver to obey provided value. This is useful with
     * texts that contain glyphs from different scripts, such as mixing latin glyphs
     * with hieroglyphs or emojis.
     * <p>
     * Text lines with different metrics will be forced to fit. This means painting
     * text in such a way that its baseline is where specified by given 'metrics'.
     * This can sometimes introduce small visual artifacs, such as taller lines
     * overpainting or being clipped by content above and below.
     * </p>
     * The possible ways to set FontMetrics include:
     * <ul>
     * <li>Obtaining 'FontMetrics' via {@link GC#getFontMetrics}. Note that this
     * will only obtain metrics for currently selected font and will not account for
     * font fallbacks (for example, with a latin font selected, painting hieroglyphs
     * usually involves a fallback font).</li>
     * <li>Obtaining 'FontMetrics' via a temporary 'TextLayout'. This would involve
     * setting a desired text sample to 'TextLayout', then measuring it with
     * {@link TextLayout#getLineMetrics(int)}. This approach will also take fallback
     * fonts into account.</li>
     * </ul>
     *
    ...
    /**
     * Sets the default font which will be used by the receiver to draw and measure
     * text. If the argument is null, then a default font appropriate for the
     * platform will be used instead. Note that a text style can override the
     * default font.
     *
    ...
    /**
     * Sets the justification of the receiver. Note that the receiver's width must
     * be set in order to use justification.
     *
     ...
     * override the default behavior of the bidirectional algorithm. Bidirectional
     * reordering can happen within a text segment but not between two adjacent
     * segments.
     * <p>
     * Each text segment is determined by two consecutive offsets in the
     * <code>segments</code> arrays. The first element of the array should always be
     * zero and the last one should always be equals to length of the text.
     * </p>
     * <p>
    ...
    /**
     * Sets the characters to be used in the segments boundaries. The segments are
     * set by calling <code>setSegments(int[])</code>. The application can use this
     * API to insert Unicode Control Characters in the text to control the display
     * of the text and bidi reordering. The characters are not accessible by any
     * other API in <code>TextLayout</code>.
     *
    ...
    /**
     * Sets the vertical indent of the receiver. The vertical indent is the space
     * left before the first line.
     *
     ...
     *                                     <ul>
     *                                     <li>ERROR_INVALID_ARGUMENT - if the
     *                                     vertical indent is negative</li>
     *                                     </ul>
     * @exception SWTException
     *                                     <ul>
     *                                     <li>ERROR_GRAPHIC_DISPOSED - if the
    ...
    /**
     * Sets the style of the receiver for the specified range. Styles previously set
     * for that range will be overwritten. The start and end offsets are inclusive
     * and will be clamped if out of range.
     *
     ...
     * in points from the origin of the text layout to the respective tab stop. The
     ...
     * @exception IllegalArgumentException
     *                                     <ul>
     *                                     <li>ERROR_INVALID_ARGUMENT - if the width
     *                                     is <code>0</code> or less than
     *                                     <code>-1</code></li>
     *                                     </ul>
     * @exception SWTException
     *                                     <ul>
    ...
    /**
     * Sets the wrap indent of the receiver. This indent is applied to all lines in
     * the paragraph except the first line.
     *
	...
	if (scriptShaprHr == OS.S_OK) {
	    ...
	    /*
	     * scriptShapeHr could have been OS.USP_E_SCRIPT_NOT_IN_FONT which indicates the
	     * whole run doesn't work with the font. The rest of this method verifies that
	     * none of the individual glyphs are missing an entry in the font. The fallback
	     * is to try other fonts (See caller)
	     */
	...
	/*
	 * The native string must remain unchanged between ScriptStringAnalyse and
	 * ScriptStringOut. According to debugging, ScriptStringAnalyse implicitly saves
	 * string to SCRIPT_STRING_ANALYSIS. Then, ScriptStringOut uses the saved string
	 * in internal call to ExtTextOutW. I believe this is due to OS.SSA_METAFILE,
	 * which is documented as follows: Write items with ExtTextOutW calls, not with
	 * glyphs. Note: passing Java chars to native function is wrong, because JNI
	 * will allocate temporary native string which will be deallocated upon return
	 * from ScriptStringAnalyse.
	 */
	...
	if (!shapeSucceed) {
	     ...
	     * when the character array is too long. The fix is to limit the size of
	     * character array to two. Note, limiting the array to only one character would
	     * cause surrogate pairs to stop working.
	     */
		...
		/*
		 * The run is composed only by white spaces, this happens when a run is split by
		 * a visual style. The font fallback for the script can not be determined using
		 * only white spaces. The solution is to use the font of the previous or next
	...
	if (!shapeSucceed) {
	    /*
	     * Shape Failed. Give up and shape the run with the default font. Missing glyphs
	     * typically will be represent as black boxes in the text.
	     */
	...
	if (run.goffsets == 0)
	    SWT.error(SWT.ERROR_NO_HANDLES);
	OS.ScriptPlace(hdc, run.psc, run.glyphs, run.glyphCount, run.visAttrs, run.analysis, run.advances, run.goffsets,
		abc);
	...
	if (style != null) {
		...
		/*
		 * Bug in Windows, on a Japanese machine, Uniscribe returns glyphcount equals
		 * zero for FFFC (possibly other unicode code points), the fix is to make sure
		 * the glyph is at least one pixel wide.
		 */
    ...
    public String toString() {
	if (isDisposed())
	    return "TextLayout_Old {*DISPOSED*}";
	return "TextLayout_Old {}";
    }
