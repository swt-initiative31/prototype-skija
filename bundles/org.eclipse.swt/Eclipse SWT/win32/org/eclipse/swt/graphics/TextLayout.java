...
public final class TextLayout extends Resource {
    ...
    private String text;
    int lineSpacingInPoints, ascent, descent, indent, wrapIndent, verticalIndentInPoints;
    boolean justify;
    int alignment;
    int[] tabs;
    int[] segments;
	...
	synchronized (textLayouts) {
		...
		surface = Surface.makeRaster(ImageInfo.makeN32Premul(b.width, b.height), 0,
...
//		return (float) (fd.getHeightF() * 2.2);
	...
	computeRuns(gc, this.selectionStart, this.selectionEnd, this.selectionForeground, this.selectionBackground, 0);
    }

    void computeRuns(GC gc, int selectionStart, int selectionEnd, Color selectionForeground, Color selectionBackground,
	    int flags) {
	...
	if (paragraph != null && this.selectionStart == selectionStart && this.selectionEnd == selectionEnd
		&& Objects.equals(this.selectionForeground, selectionForeground)
		&& Objects.equals(this.selectionBackground, selectionBackground))
	    return;
	...
	LineMetrics[] lineMetrics = paragraph.getLineMetrics();
	{
		...
		for (LineMetrics metrics : paragraph.getLineMetrics()) {
		    System.out.println("Zeile Offset Y: " + metrics.getBaseline());
		    System.out.println("Zeilenhöhe: " + metrics.getHeight());

		    double ascend = metrics.getAscent();
		    double baseline = metrics.getBaseline();
		    double top = baseline - ascend;
		    System.out.println("Der höchste Punkt der Zeile: " + top);
		...
		for (int k = 0; k < count; k++) {
		    ...
		    double ascend = m.getAscent();
		    double descent = m.getDescent();
		    double baseline = m.getBaseline();
		    double top = baseline - ascend;
		    double bottom = baseline + descent;
	...
	io.github.humbleui.skija.Image im = surface.makeImageSnapshot(new IRect(0, 0, b.width, b.height));
	...
	computeRuns(gc, selectionStart, selectionEnd, selectionForeground, selectionBackground, flags);
	int length = translateOffset(text.length());
	if (length == 0 && flags == 0)
	    return;

	if (imageBytes != null) {
	    org.eclipse.swt.graphics.Image transferImage = new org.eclipse.swt.graphics.Image(gc.getDevice(),
    ...
    private synchronized Paragraph createParagraph(int selectionStart, int selectionEnd, Color selectionForeground,
	    Color selectionBackground) {
	...
	if (selectionForeground == null)
	    selectionForeground = device.getSystemColor(SWT.COLOR_LIST_FOREGROUND);
	...
	style.setAlignment(getAlignmentStyle());
	...
	io.github.humbleui.skija.paragraph.TextStyle normal = new io.github.humbleui.skija.paragraph.TextStyle()
		.setFontSize(getFontSize()).setFontFamilies(new String[] { fd.getName() }).setColor(0xFF000000);

	io.github.humbleui.skija.paragraph.TextStyle selectionStyle = new io.github.humbleui.skija.paragraph.TextStyle()
		.setFontSize(getFontSize()).setFontFamilies(new String[] { fd.getName() })
	...
	try (ParagraphBuilder paragraphBuilder = new ParagraphBuilder(style, fc)) {
		...
		if (!"".equals(s)) {
			...
			for (int i = si.start; i < nextStyleStart; i++) {
				...
				addText(paragraphBuilder, tabPlaceholder,
					str.substring(i, Math.min(nextStyleStart, selectionEnd + 1)));

				paragraphBuilder.popStyle();
				i = Math.min(nextStyleStart, selectionEnd + 1) - 1;

			    } else {
				if (i > selectionStart) {
				    paragraphBuilder.pushStyle(ts);

				    addText(paragraphBuilder, tabPlaceholder, str.substring(i, nextStyleStart));
				    ...
				    addText(paragraphBuilder, tabPlaceholder,
					    str.substring(i, Math.min(nextStyleStart, selectionStart)));

				    paragraphBuilder.popStyle();
				    i = Math.min(nextStyleStart, selectionStart) - 1;
				}
    ...
    private Alignment getAlignmentStyle() {
    ...
    private void addText(ParagraphBuilder paragraphBuilder, PlaceholderStyle tabPlaceholder, String substring) {
    ...
    private io.github.humbleui.skija.paragraph.TextStyle convertToTextStyle(StyleItem si, FontData fd) {
	...
	if (ts == null) {
	    ...
	    return new io.github.humbleui.skija.paragraph.TextStyle().setFontSize(getFontSize())
		    .setFontFamilies(new String[] { fd.getName() }).setForeground(foreP);

	}

	int foreground = SkijaGC.convertSWTColorToSkijaColor(
		ts.foreground != null ? ts.foreground : device.getSystemColor(SWT.COLOR_BLACK));
	Paint foreP = new Paint().setColor(foreground);
	...
	if (ts.font != null && ts.font.getFontData() != null && ts.font.getFontData().length >= 1) {
	...
	io.github.humbleui.skija.paragraph.TextStyle textSty = new io.github.humbleui.skija.paragraph.TextStyle()
		.setFontStyle(fs).setFontSize(fontSize).setFontFamilies(new String[] { fd.getName() })
		.setForeground(foreP) //
		.setBackground(backP);
	...
	if (lineBounds == null && fastCalculationMode) {
	    ...
	    int height = Math.abs(fm.getAscent()) + Math.abs(fm.getDescent()) + Math.abs(fm.getLeading());
	    ...
	    if (wrapWidth == -1) {
		return new Rectangle(0, 0, (int) (maxChars * avgWidth) + 20, height * linesCount);
	    } else {
	...
	return this.bidi.getLevelAt(offset);
	...
	if (lineBounds == null && lineIndex == 0 && fastCalculationMode) {
	    ...
	    return new Rectangle(0, 0, 0,
		    Math.abs(fm.getAscent()) + Math.abs(fm.getDescent()) + Math.abs(fm.getLeading()));

	}
	computeRuns(null);
	if (lineIndex < 0)
	    throw new IllegalArgumentException("Invalid lineIndex: " + lineIndex + ". LineBounds size: "
		    + (lineBounds != null ? lineBounds.length : null));

	if (lineIndex >= lineBounds.length)
	    throw new IllegalArgumentException("Invalid lineIndex: " + lineIndex + ". LineBounds size: "
		    + (lineBounds != null ? lineBounds.length : null));
    ...
    private static int findNextWordStart(String input, int position) {
	// Regex zur Suche nach einem Wort an einer beliebigen Stelle im Text
	String regex = "\\s[^\\s]";
	java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(regex);
	java.util.regex.Matcher matcher = pattern.matcher(input);

	// Beginne die Suche ab der gewünschten Position
	if (position < input.length()) {
	    matcher.region(position, input.length());

	    // Durch Iterieren die nächste Wortgrenzen finden
	    while (matcher.find()) {
		// the first is the whitespace, the second the not whitespace
	...
	if (movement == SWT.MOVEMENT_WORD_START) {
	    ...
	    // move to the next word start
	    var i = findNextWordStart(text, offset);
	    if (i == -1)
		return text.length();
	    else
		return i;
	...
	PositionWithAffinity affinity = paragraph.getGlyphPositionAtCoordinate((float) x, (float) y);
	...
	if (textDirection == -1 || textDirection == SWT.NONE)
	...
	this.swtFont = font;
	this.nativeZoom = this.swtFont == null ? nativeZoom : this.swtFont.zoom;
	...
	if ((this.orientation & SWT.LEFT_TO_RIGHT) != 0 || (this.orientation & SWT.RIGHT_TO_LEFT) != 0)
	    this.textDirection = this.orientation;
	...
	freeRuns();
	this.text = text;
	this.bidi = new Bidi(this.text, Bidi.DIRECTION_DEFAULT_LEFT_TO_RIGHT);
