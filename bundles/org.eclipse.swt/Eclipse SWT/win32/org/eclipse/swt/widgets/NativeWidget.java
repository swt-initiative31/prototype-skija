...
void maybeEnableDarkSystemTheme(long handle) {
    if (getWidget().display.useDarkModeExplorerTheme) {
...
public void dispose () {
	...
	*/
	if (getWidget().isDisposed())
	    return;
	if (!getWidget().isValidThread())
	    error(SWT.ERROR_THREAD_INVALID_ACCESS);
	getWidget().release(true);
}
...
public <L extends EventListener> Stream<L> getTypedListeners (int eventType, Class<L> listenerType) {
    return Arrays.stream(getWidget().getListeners(eventType)) //
...
boolean sendMouseEvent(int type, int button, int count, int detail, boolean send, long hwnd, long lParam) {
    if (!getWidget().hooks(type) && !getWidget().filters(type))
	return true;
    Event event = new Event();
    event.button = button;
    event.detail = detail;
    event.count = count;
    int zoom = getWidget().getZoom();
    event.setLocation(DPIUtil.scaleDown(OS.GET_X_LPARAM(lParam), zoom),
	    DPIUtil.scaleDown(OS.GET_Y_LPARAM(lParam), zoom));
    setInputState(event, type);
    mapEvent(hwnd, event);
    if (send) {
	getWidget().sendEvent(type, event);
	if (getWidget().isDisposed())
	    return false;
    } else {
	getWidget().postEvent(type, event);
    }
    return event.doit;
}
...
boolean sendMouseWheelEvent (int type, long hwnd, long wParam, long lParam) {
	...
	MouseWheelData wheelData = new MouseWheelData(vertical, null, wParam, getWidget().display.scrollRemainderEvt);
...
public void setData (String key, Object value) {
	...
	if (key.equals(SWT.SKIN_CLASS) || key.equals(SWT.SKIN_ID))
	    getWidget().reskin(SWT.ALL);
}

boolean sendFocusEvent(int type) {
    getWidget().sendEvent(type);
...
boolean setKeyState (Event event, int type, long wParam, long lParam) {
	...
	*/
	...
	switch (getWidget().display.lastAscii) {
		case SWT.DEL:
	    if (getWidget().display.lastKey == SWT.BS)
		getWidget().display.lastAscii = SWT.BS;
			break;
		case SWT.LF:
		    if (getWidget().display.lastKey == SWT.CR)
			getWidget().display.lastAscii = SWT.CR;
			break;
	}
	...
	*/
	if (getWidget().display.lastKey == SWT.CR && getWidget().display.lastAscii == SWT.CR) {
	    if ((lParam & 0x1000000) != 0)
		getWidget().display.lastKey = SWT.KEYPAD_CR;
	}

	setLocationMask(event, type, wParam, lParam);

	if (getWidget().display.lastVirtual) {
		/*
...
LRESULT wmCaptureChanged (long hwnd, long wParam, long lParam) {
    getWidget().display.captureChanged = true;
	return null;
}
...
boolean sendMouseEvent(int type, int button, long hwnd, long lParam) {
    return sendMouseEvent(type, button, getWidget().display.getClickCount(type, button, hwnd, lParam), 0, false, hwnd,
	    lParam);
}

LRESULT wmChar (long hwnd, long wParam, long lParam) {
    getWidget().display.lastAscii = (int) wParam;
    if (!getWidget().sendKeyEvent(SWT.KeyDown, OS.WM_CHAR, wParam, lParam)) {
		return LRESULT.ONE;
	}
	// widget could be disposed at this point
	return null;
...
LRESULT wmContextMenu (long hwnd, long wParam, long lParam) {
	...
	*/
	...
	/* Show the menu */
	return getWidget().showMenu(x, y, detail) ? LRESULT.ZERO : null;
}
...
LRESULT wmKeyDown (long hwnd, long wParam, long lParam) {
	...
	*/
	...
	if (OS.PeekMessage (msg, hwnd, OS.WM_DEADCHAR, OS.WM_DEADCHAR, flags)) {
	    getWidget().display.lastDead = true;
	    getWidget().display.lastVirtual = mapKey == 0;
	    getWidget().display.lastKey = getWidget().display.lastVirtual ? (int) wParam : mapKey;
		return null;
	}
	...
	*/
	getWidget().display.lastVirtual = mapKey == 0 || getWidget().display.numpadKey((int) wParam) != 0;
	if (getWidget().display.lastVirtual) {
	    getWidget().display.lastKey = (int) wParam;
		/*
		* Feature in Windows.  The virtual key VK_DELETE is not
		* treated as both a virtual key and an ASCII key by Windows.
		* Therefore, we will not receive a WM_CHAR for this key.
...
LRESULT wmKeyUp (long hwnd, long wParam, long lParam) {
	...
	*/
	if (!getWidget().hooks(SWT.KeyUp) && !display.filters(SWT.KeyUp)) {
		display.lastKey = display.lastAscii = 0;
		display.lastVirtual = display.lastDead = false;
		return null;
	}
...
LRESULT wmKillFocus (long hwnd, long wParam, long lParam) {
	...
	getWidget().sendFocusEvent(SWT.FocusOut);
	// widget could be disposed at this point
...
LRESULT wmLButtonDblClk (long hwnd, long wParam, long lParam) {
	...
	*/
	...
	if (!display.captureChanged && !getWidget().isDisposed()) {
...
LRESULT wmLButtonDown (long hwnd, long wParam, long lParam) {
	...
	if (count == 1 && (getWidget().state & DRAG_DETECT) != 0 && getWidget().hooks(SWT.DragDetect)) {
		/*
...
LRESULT wmMouseLeave (long hwnd, long wParam, long lParam) {
    getWidget().state &= ~MOUSE_OVER;
    if (!getWidget().hooks(SWT.MouseExit) && !getWidget().filters(SWT.MouseExit))
	return null;
	int pos = OS.GetMessagePos ();
	POINT pt = new POINT ();
...
LRESULT wmMouseMove (long hwnd, long wParam, long lParam) {
	...
	if (pos != display.lastMouse || display.captureChanged) {
	    boolean trackMouse = (getWidget().state & TRACK_MOUSE) != 0;
	    boolean mouseEnter = getWidget().hooks(SWT.MouseEnter) || display.filters(SWT.MouseEnter);
	    boolean mouseExit = getWidget().hooks(SWT.MouseExit) || display.filters(SWT.MouseExit);
	    boolean mouseHover = getWidget().hooks(SWT.MouseHover) || display.filters(SWT.MouseHover);
		if (trackMouse || mouseEnter || mouseExit || mouseHover) {
			...
			if (mouseEnter && (getWidget().state & MOUSE_OVER) == 0) {
				/*
...
LRESULT wmPaint (long hwnd, long wParam, long lParam) {
	...
	if (!getWidget().hooks(SWT.Paint) && !getWidget().filters(SWT.Paint)) {
		return null;
	}
	...
	if (gc != null) {
		...
		if (width != 0 && height != 0) {
			...
			Event event = new Event ();
			event.gc = gc;
			event.setBounds(DPIUtil.scaleDown(new Rectangle(rect.left, rect.top, width, height),
				getWidget().getZoom()));
			getWidget().sendEvent(SWT.Paint, event);
			// widget could be disposed at this point
			event.gc = null;
		}
		gc.dispose ();
	...
	if ((lParam & OS.PRF_NONCLIENT) != 0) {
		if (OS.IsAppThemed ()) {
			...
			if ((bits & OS.WS_EX_CLIENTEDGE) != 0) {
				...
				OS.DrawThemeBackground(getWidget().display.hEditTheme(getWidget().nativeZoom), wParam,
...
LRESULT wmSetFocus (long hwnd, long wParam, long lParam) {
	...
	getWidget().sendFocusEvent(SWT.FocusIn);
	// widget could be disposed at this point
...
LRESULT wmSysChar (long hwnd, long wParam, long lParam) {
	...
	if (!getWidget().hooks(SWT.KeyDown) && !display.filters(SWT.KeyDown)) {
		return null;
	}
...
GC createNewGC(long hDC, GCData data) {
    data.nativeZoom = getWidget().nativeZoom;
...
int getSystemMetrics(int nIndex) {
	...
	if (OS.WIN32_BUILD >= OS.WIN32_BUILD_WIN10_1607) {
	    return OS.GetSystemMetricsForDpi(nIndex, DPIUtil.mapZoomToDPI(getWidget().nativeZoom));
	}
