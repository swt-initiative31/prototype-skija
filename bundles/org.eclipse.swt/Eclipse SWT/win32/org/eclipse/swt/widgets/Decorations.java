...
public class Decorations extends Canvas {
	...
	void bringToTop() {
		/*
		 * This code is intentionally commented. On some platforms, the ON_TOP style
		 * creates a shell that will stay on top of every other shell on the desktop.
		 * Using SetWindowPos () with HWND_TOP caused problems on Windows 98 so this
		 * code is commented out until this functionality is specified and the problems
		 * are fixed.
		 */
...
//	}
	...
	static int checkStyle(int style) {
		...
		/*
		 * Both WS_SYSMENU and WS_CAPTION must be set in order to for the system menu to
		 * appear.
		 */
	...
	Rectangle getClientAreaInPixels() {
		...
		if (OS.IsIconic(handle)) {
			...
			int height = lpwndpl.bottom - lpwndpl.top;
			/*
			 * Feature in Windows. For some reason WM_NCCALCSIZE does not compute the client
			 * area when the window is minimized. The fix is to compute it using
			 * AdjustWindowRectEx() and GetSystemMetrics().
			 *
			 * NOTE: This code fails to compute the correct client area for a minimized
			 * window where the menu bar would wrap were the window restored. There is no
	 ...
	 * manager when the instance is marked as iconified, and may also be displayed
	 * somewhere in the trim when the instance is in normal or maximized states.
	 * <p>
	 * Note: This method will return null if called before <code>setImage()</code>
	 * is called. It does not provide access to a window manager provided, "default"
	 * image even if one exists.
	 * </p>
	 *
	 * @return the image
	 *
	 ...
	 * manager when the instance is marked as iconified, and may also be displayed
	 * somewhere in the trim when the instance is in normal or maximized states.
	 * Depending where the icon is displayed, the platform chooses the icon with the
	 * "best" attributes. It is expected that the array will contain the same icon
	 * rendered at different sizes, with different depth and transparency
	 * attributes.
	 *
	 * <p>
	 * Note: This method will return an empty array if called before
	 * <code>setImages()</code> is called. It does not provide access to a window
	...
	/**
	 * Returns <code>true</code> if the receiver is currently maximized, and false
	 * otherwise.
	 *
	...
	public boolean getMaximized() {
		...
		if (!SWT.NATIVE_DECORATIONS && this instanceof Shell) {
	...
	/**
	 * Returns <code>true</code> if the receiver is currently minimized, and false
	 * otherwise.
	 *
	...
	/**
	 * Returns the receiver's text, which is the string that the window manager will
	 * typically display as the receiver's <em>title</em>. If the text has not
	...
	public boolean isReparentable() {
		checkWidget();
		/*
		 * Feature in Windows. Calling SetParent() for a shell causes a kind of fake MDI
		 * to happen. It doesn't work well on Windows and is not supported on the other
		 * platforms. The fix is to disallow the SetParent().
		 */
		return false;
	}
	 ...
	 * argument, and if the argument is null, sets the receiver's default button to
	 * the first button which was set as the receiver's default button (called the
	 * <em>saved default button</em>). If no default button had previously been set,
	 * or the saved default button was disposed, the receiver's default button will
	 * be set to null.
	 * <p>
	 * The default button is the button that is selected when the receiver is active
	...
	/**
	 * Sets the receiver's image to the argument, which may be null. The image is
	 * typically displayed by the window manager when the instance is marked as
	 * iconified, and may also be displayed somewhere in the trim when the instance
	 * is in normal or maximized states.
	 *
	...
	void setImages(Image image, Image[] images) {
		...
		/*
		 * Bug in Windows. When WM_SETICON is used to remove an icon from the window
		 * trimmings for a window with the extended style bits WS_EX_DLGMODALFRAME, the
		 * window trimmings do not redraw to hide the previous icon. The fix is to force
		 * a redraw.
		 */
	 ...
	 * marked as iconified, and may also be displayed somewhere in the trim when the
	 * instance is in normal or maximized states. Depending where the icon is
	 * displayed, the platform chooses the icon with the "best" attributes. It is
	 ...
	 *                                     the images is null or has been
	 *                                     disposed</li>
	 *                                     </ul>
	 * @exception SWTException
	 *                                     <ul>
	...
	public void setMaximized(boolean maximized) {
		...
		if (!SWT.NATIVE_DECORATIONS && this instanceof Shell) {
			...
			if (maximized) {
				...
				this.previousBounds = getBounds();

				Monitor primary = this.getMonitor();
				Rectangle bounds = primary.getBounds();
				Rectangle clientArea = primary.getClientArea();

				int taskBarHeight = bounds.height - clientArea.height;

				var maximizedBounds = new Rectangle(bounds.x, bounds.y, bounds.width, bounds.height - taskBarHeight);

				if (this.previousBounds.equals(maximizedBounds)) {
				...
				if (this.previousBounds != null) {
					...
					setBounds(bounds.x + 10, bounds.y + 10, bounds.width - 20, bounds.height - taskBarHeight - 20);
					this.maximized = false;
					return;
				}
			}
	...
	void setParent() {
		/*
		 * In order for an MDI child window to support a menu bar, setParent () is
		 * needed to reset the parent. Otherwise, the MDI child window will appear as a
		 * separate shell. This is an undocumented and possibly dangerous Windows
		 * feature.
		 */
	...
	public void setVisible(boolean visible) {
		...
		if (visible) {
			...
			if (!getDrawing()) {
				...
				/*
				 * Bug in Windows. On Vista using the Classic theme, when the window is hung and
				 * UpdateWindow() is called, nothing is drawn, and outstanding WM_PAINTs are
				 * cleared. This causes pixel corruption. The fix is to avoid calling update on
				 * hung windows.
				 */
	...
	int widgetStyle() {
		/*
		 * Clear WS_VISIBLE and WS_TABSTOP. NOTE: In Windows, WS_TABSTOP has the same
		 * value as WS_MAXIMIZEBOX so these bits cannot be used to control tabbing.
		 */
