...
public class Decorations extends Canvas implements ICustomWidget {
	...
	void bringToTop() {
		/*
		 * This code is intentionally commented. On some platforms, the ON_TOP
		 * style creates a shell that will stay on top of every other shell on
		 * the desktop. Using SetWindowPos () with HWND_TOP caused problems on
		 * Windows 98 so this code is commented out until this functionality is
		 * specified and the problems are fixed.
		 */
	...
	static int checkStyle(int style) {
		if ((style & SWT.NO_TRIM) != 0) {
			style &= ~(SWT.CLOSE | SWT.TITLE | SWT.MIN | SWT.MAX | SWT.RESIZE
					| SWT.BORDER);
	...
	int compare(ImageData data1, ImageData data2, int width, int height,
			int depth) {
		int value1 = Math.abs(data1.width - width),
				value2 = Math.abs(data2.width - width);
		if (value1 == value2) {
		...
		if (hasMenu) {
			...
			while ((testRect.bottom - testRect.top) < height) {
				if (testRect.bottom - testRect.top == 0)
					break;
				rect.top -= getSystemMetrics(OS.SM_CYMENU)
						- getSystemMetrics(OS.SM_CYBORDER);
				OS.SetRect(testRect, 0, 0, rect.right - rect.left,
						rect.bottom - rect.top);
				OS.SendMessage(handle, OS.WM_NCCALCSIZE, 0, testRect);
			}
	...
	Rectangle getClientAreaInPixels() {
		...
		if (OS.IsIconic(handle)) {
			...
			int height = lpwndpl.bottom - lpwndpl.top;
			/*
			 * Feature in Windows. For some reason WM_NCCALCSIZE does not
			 * compute the client area when the window is minimized. The fix is
			 * to compute it using AdjustWindowRectEx() and GetSystemMetrics().
			 *
			 * NOTE: This code fails to compute the correct client area for a
			 * minimized window where the menu bar would wrap were the window
			 * restored. There is no fix for this problem at this time.
			 */
			if (horizontalBar != null)
	...
	void setBoundsInPixels(int x, int y, int width, int height, int flags,
		...
		if ((OS.SWP_NOSIZE & flags) == 0) {
			sameExtent = rect.right - rect.left == width
	...
	void setImages(Image image, Image[] images) {
		...
		if (image != null) {
			...
			if (images != null && images.length > 0) {
				...
				if (images.length > 1) {
					...
					images = bestImages;
					sort(images, datas, getSystemMetrics(OS.SM_CXSMICON),
							getSystemMetrics(OS.SM_CYSMICON), depth);
				}
				smallIcon = images[0];
				if (images.length > 1) {
					sort(images, datas, getSystemMetrics(OS.SM_CXICON),
							getSystemMetrics(OS.SM_CYICON), depth);
				}
				largeIcon = images[0];
			}
		}
		...
		/*
		 * Bug in Windows. When WM_SETICON is used to remove an icon from the
		 * window trimmings for a window with the extended style bits
		 * WS_EX_DLGMODALFRAME, the window trimmings do not redraw to hide the
		 * previous icon. The fix is to force a redraw.
		 */
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if the menu has been
	 *                disposed</li>
	...
	void setParent() {
		/*
		 * In order for an MDI child window to support a menu bar, setParent ()
		 * is needed to reset the parent. Otherwise, the MDI child window will
		 * appear as a separate shell. This is an undocumented and possibly
		 * dangerous Windows feature.
		 */
	...
	void setPlacement(int x, int y, int width, int height, int flags) {
		...
		if ((flags & OS.SWP_NOSIZE) == 0) {
			sameExtent = lpwndpl.right - lpwndpl.left != width
	...
	public void setVisible(boolean visible) {
		...
		if (visible) {
			...
			if (!getDrawing()) {
				...
				if (lpStartUpInfo != null && (lpStartUpInfo.dwFlags
						& OS.STARTF_USESHOWWINDOW) != 0) {
				...
				/*
				 * Bug in Windows. On Vista using the Classic theme, when the
				 * window is hung and UpdateWindow() is called, nothing is
				 * drawn, and outstanding WM_PAINTs are cleared. This causes
				 * pixel corruption. The fix is to avoid calling update on hung
				 * windows.
				 */
	...
	void sort(Image[] images, ImageData[] datas, int width, int height,
			int depth) {
		...
		for (int gap = length / 2; gap > 0; gap /= 2) {
			for (int i = gap; i < length; i++) {
				for (int j = i - gap; j >= 0; j -= gap) {
					if (compare(datas[j], datas[j + gap], width, height,
							depth) >= 0) {
						Image swap = images[j];
	...
	boolean translateMDIAccelerator(MSG msg) {
		if (!(this instanceof Shell)) {
			...
			if (hwndMDIClient != 0
					&& OS.TranslateMDISysAccel(hwndMDIClient, msg)) {
				return true;
			}
	...
	int widgetStyle() {
		/*
		 * Clear WS_VISIBLE and WS_TABSTOP. NOTE: In Windows, WS_TABSTOP has the
		 * same value as WS_MAXIMIZEBOX so these bits cannot be used to control
		 * tabbing.
		 */
	...
	LRESULT WM_ACTIVATE(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Feature in AWT. When an AWT Window is activated, for some reason, it
		 * seems to forward the WM_ACTIVATE message to the parent. Normally, the
		 * parent is an AWT Frame. When AWT is embedded in SWT, the SWT shell
		 * gets the WM_ACTIVATE and assumes that it came from Windows. When an
		 * SWT shell is activated it restores focus to the last control that had
		 * focus. If this control is an embedded composite, it takes focus from
		 * the AWT Window. The fix is to ignore WM_ACTIVATE messages that come
		 * from AWT Windows.
		 */
		...
		if (loWord != 0) {
			/*
			 * When the high word of wParam is non-zero, the activation state of
			 * the window is being changed while the window is minimized. If
			 * this is the case, do not report activation events or restore the
			 * focus.
			 */
			...
			if (control == null || control instanceof Shell) {
				if (this instanceof Shell) {
					Event event = new Event();
					event.detail = loWord == OS.WA_CLICKACTIVE
							? SWT.MouseDown
							: SWT.None;
	...
	private static void handleDPIChange(Widget widget, int newZoom,
		...
		DPIZoomChangeRegistry.applyChange(decorations.getMenuBar(), newZoom,
				scalingFactor);
