...
public int getTextLimit () {
	checkWidget ();
	long hwndText = OS.GetDlgItem (handle, CBID_EDIT);
	if (hwndText == 0) return Combo.LIMIT;
	int limit = (int)OS.SendMessage (hwndText, OS.EM_GETLIMITTEXT, 0, 0) & 0x7FFFFFFF;
	if (segments != null && limit < Combo.LIMIT) limit = Math.max (1, limit - segments.length);
	return limit;
}
...
public void setTextLimit (int limit) {
	...
	if (segments != null && limit > 0) {
		OS.SendMessage (handle, OS.CB_LIMITTEXT, limit + Math.min (segments.length, Combo.LIMIT - limit), 0);
	} else {
