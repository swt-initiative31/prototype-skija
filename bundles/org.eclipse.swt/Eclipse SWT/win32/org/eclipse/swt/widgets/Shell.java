...
public class Shell extends Decorations {
	Menu activeMenu;
	ToolTip[] toolTips;
	long hwndMDIClient, lpstrTip, toolTipHandle, balloonTipHandle,
			menuItemToolTipHandle;
	int minWidth = SWT.DEFAULT, minHeight = SWT.DEFAULT, maxWidth = SWT.DEFAULT,
	...
	// small modifications in Display were necessary to move the callback back
	// to
	// this window class
	private Display d;
	// handle
	...
	final static int[] SYSTEM_COLORS = {OS.COLOR_BTNFACE, OS.COLOR_WINDOW,
			OS.COLOR_BTNTEXT, OS.COLOR_WINDOWTEXT, OS.COLOR_HIGHLIGHT,
			OS.COLOR_SCROLLBAR,};
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(Shell::handleDPIChange,
				Shell.class);
	}
	...
	private void onDispose(Event e) {
		...
		// we have a single window, so we close the Display now in order to quit
		// the
		// app.
	...
	private void onResize(Event e) {
		...
		if (SWT.USE_OPENGL) {
			...
			surface = Surface.makeFromBackendRenderTarget(skijaContext,
	...
	private void setupMemoryDC() {
		...
		if (withSkija) {
			...
			hBitmap = OS.CreateDIBSection(hdc, bmi, OS.DIB_RGB_COLORS, pBits, 0,
					0);
			if (hBitmap == 0)
		...
		OS.MoveMemory(bmi, bmiHeader, BITMAPINFOHEADER.sizeof);
		long[] pBits = new long[1];
		long hBitmap = OS.CreateDIBSection(hdc, bmi, OS.DIB_RGB_COLORS, pBits,
				0, 0);
		if (hBitmap == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
	...
	protected void doPaint() {
		...
		if (SWT.USE_OPENGL) {
			...
			if (directColor) {
				long hBrush = OS
						.CreateSolidBrush(convertRGBToInt(new RGB(255, 0, 0))); // Rot
				RECT rect = new RECT(100, 100, 400, 300);
				...
				// for vsync use DwmFlush in order to wait until the screen
				// drawing is finished.
				if (vsync)
					OS.DwmFlush();
	...
	private void drawSurface() {
		...
		try (var paint = new Paint()) {
			...
			surface.getCanvas().drawCircle((int) (position * size.x), 100, 100,
					paint);
		}
	}
	 ...
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if the parent is
	 *                disposed</li>
	 *                </ul>
	 * @exception SWTException
	 *                <ul>
	...
	public void addShellListener(ShellListener listener) {
		addTypedListener(listener, SWT.Close, SWT.Iconify, SWT.Deiconify,
	...
	long callWindowProc(long hwnd, int msg, long wParam, long lParam) {
		...
		if ((style & SWT.TOOL) != 0) {
			int trim = SWT.TITLE | SWT.CLOSE | SWT.MIN | SWT.MAX | SWT.BORDER
					| SWT.RESIZE;
			if ((style & trim) == 0)
	...
	void center() {
		...
		Rectangle parentRect = display.mapInPixels(parent, null,
				parent.getClientAreaInPixels());
		int x = Math.max(parentRect.x,
				parentRect.x + (parentRect.width - rect.width) / 2);
		int y = Math.max(parentRect.y,
	...
	void createBalloonTipHandle() {
		balloonTipHandle = OS.CreateWindowEx(0,
				new TCHAR(0, OS.TOOLTIPS_CLASS, true), null,
		...
		if (ToolTipProc == 0) {
			ToolTipProc = OS.GetWindowLongPtr(balloonTipHandle,
					OS.GWLP_WNDPROC);
		}
		/*
		...
		OS.SetWindowLongPtr(balloonTipHandle, OS.GWLP_WNDPROC,
				display.windowProc);
	}
	...
	void createHandle() {
		...
		/*
		 * On Windows 98 and NT, setting a window to be the top most window
		 * using HWND_TOPMOST can result in a parent dialog shell being moved
		 * behind its parent if the dialog has a sibling that is currently on
		 * top This only occurs using SetWindowPos (), not when the handle is
		 * created.
		 */
		/*
		...
		if (!embedded) {
			 ...
			 * SetWindowLong(), Windows does not resize the client area of the
			 * window to get rid of the caption until the first resize. The fix
			 * is to use SetWindowPos() with SWP_DRAWFRAME to force the frame to
			 * be redrawn and resized.
			 */
	...
	long createToolTipHandle(long parent) {
		long toolTipHandle = OS.CreateWindowEx(0,
				new TCHAR(0, OS.TOOLTIPS_CLASS, true), null,
				OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX, OS.CW_USEDEFAULT, 0,
				OS.CW_USEDEFAULT, 0, parent, 0, OS.GetModuleHandle(null), null);
		if (toolTipHandle == 0)
			error(SWT.ERROR_NO_HANDLES);
		if (ToolTipProc == 0) {
	...
	void fixActiveShell() {
		/*
		 * Feature in Windows. When the active shell is disposed or hidden,
		 * Windows normally makes the parent shell active and assigns focus.
		 * This does not happen when the parent shell is disabled. Instead,
		 * Windows assigns focus to the next shell on the desktop (possibly a
		 * shell in another application). The fix is to activate the disabled
		 * parent shell before disposing or hiding the active shell.
		 */
		long hwndParent = OS.GetParent(handle);
		if (hwndParent != 0 && handle == OS.GetActiveWindow()) {
			if (!OS.IsWindowEnabled(hwndParent)
					&& OS.IsWindowVisible(hwndParent)) {
	...
	void fixToolTip() {
		/*
		 * Bug in Windows. On XP, when a tooltip is hidden due to a time out or
		 * mouse press, the tooltip remains active although no longer visible
		 * and won't show again until another tooltip becomes active. If there
		 * is only one tooltip in the window, it will never show again. The fix
		 * is to remove the current tooltip and add it again every time the
		...
		TOOLINFO lpti = new TOOLINFO();
		lpti.cbSize = TOOLINFO.sizeof;
		if (OS.SendMessage(toolTipHandle, OS.TTM_GETCURRENTTOOL, 0,
				lpti) != 0) {
		...
		if (OS.SendMessage(menuItemToolTipHandle, OS.TTM_GETCURRENTTOOL, 0,
				lptiMt) != 0) {
			if ((lptiMt.uFlags & OS.TTF_IDISHWND) != 0) {
				OS.SendMessage(menuItemToolTipHandle, OS.TTM_DELTOOL, 0,
						lptiMt);
				OS.SendMessage(menuItemToolTipHandle, OS.TTM_ADDTOOL, 0,
						lptiMt);
			}
		}
	}
	...
	ToolTip getCurrentToolTip(long hwndToolTip) {
		...
		if (OS.SendMessage(hwndToolTip, OS.TTM_GETCURRENTTOOL, 0, 0) != 0) {
			TOOLINFO lpti = new TOOLINFO();
			lpti.cbSize = TOOLINFO.sizeof;
			if (OS.SendMessage(hwndToolTip, OS.TTM_GETCURRENTTOOL, 0,
					lpti) != 0) {
				if ((lpti.uFlags & OS.TTF_IDISHWND) == 0)
	...
	public int getImeInputMode() {
		...
		if (open)
			open = OS.ImmGetConversionStatus(hIMC, lpfdwConversion,
					lpfdwSentence);
	...
	/**
	 * Returns a point describing the maximum receiver's size. The x coordinate
	 * of the result is the maximum width of the receiver. The y coordinate of
	 * the result is the maximum height of the receiver.
	 *
	...
	long hwndMDIClient() {
		if (hwndMDIClient == 0) {
			...
			hwndMDIClient = OS.CreateWindowEx(0,
					new TCHAR(0, "MDICLIENT", true), null, widgetStyle, 0, 0, 0,
					0, handle, 0, OS.GetModuleHandle(null), new CREATESTRUCT());
	...
	public void open() {
		...
		if (lpStartUpInfo == null
				|| (lpStartUpInfo.dwFlags & OS.STARTF_USESHOWWINDOW) == 0) {
			bringToTop();
			if (isDisposed())
				return;
		}
		...
		if (isDisposed())
			return;
		/*
		 * Bug in Windows XP. Despite the fact that an icon has been set for a
		 * window, the task bar displays the wrong icon the first time the
		 * window is made visible with ShowWindow() after a call to
		 * BringToTop(), when a long time elapses between the ShowWindow() and
		 * the time the event queue is read. The icon in the window trimming is
		 * correct but the one in the task bar does not get updated. The fix is
		 * to call PeekMessage() with the flag PM_NOREMOVE and PM_QS_SENDMESSAGE
		 * to respond to a cross thread WM_GETICON.
		 *
		 * NOTE: This allows other cross thread messages to be delivered, most
		 * notably WM_ACTIVATE.
		 */
		MSG msg = new MSG();
		...
		/*
		 * When no widget has been given focus, or another push button has
		 * focus, give focus to the default button. This avoids overriding the
		 * default button.
		 */
	...
	void setActiveControl(Control control, int type) {
		 ...
		 */
		Control[] activate = (control == null)
				? new Control[0]
				: control.getPath();
		Control[] deactivate = (lastActive == null)
				? new Control[0]
				: lastActive.getPath();
		lastActive = control;
	...
	public void setAlpha(int alpha) {
		...
		if (alpha == 0xFF) {
			...
			int flags = OS.RDW_ERASE | OS.RDW_INVALIDATE | OS.RDW_FRAME
					| OS.RDW_ALLCHILDREN;
			OS.RedrawWindow(handle, null, 0, flags);
		} else {
			OS.SetWindowLong(handle, OS.GWL_EXSTYLE, bits | OS.WS_EX_LAYERED);
			OS.SetLayeredWindowAttributes(handle, 0, (byte) alpha,
					OS.LWA_ALPHA);
		}
	}
	...
	public Rectangle getBounds() {
		if (getDisplay().isRescalingAtRuntime()) {
			...
			return display
					.translateRectangleInPointsInDisplayCoordinateSystemByContainment(
							boundsInPixels.x, boundsInPixels.y,
							boundsInPixels.width, boundsInPixels.height);
		}
	...
	public Point getLocation() {
		if (getDisplay().isRescalingAtRuntime()) {
			...
			return display.translateLocationInPointInDisplayCoordinateSystem(
					locationInPixels.x, locationInPixels.y);
		}
	...
	public void setLocation(int x, int y) {
		if (getDisplay().isRescalingAtRuntime()) {
			Point location = display
					.translateLocationInPixelsInDisplayCoordinateSystem(x, y);
	...
	public void setBounds(int x, int y, int width, int height) {
		if (getDisplay().isRescalingAtRuntime()) {
			Rectangle boundsInPixels = display
					.translateRectangleInPixelsInDisplayCoordinateSystemByContainment(
							x, y, width, height);
			setBoundsInPixels(boundsInPixels.x, boundsInPixels.y,
					boundsInPixels.width, boundsInPixels.height);
		} else {
	...
	void setBoundsInPixels(int x, int y, int width, int height, int flags,
		...
		if (fullScreen)
			setFullScreen(false);
		/*
		 * Bug in Windows. When a window has alpha and SetWindowPos() is called
		 * with SWP_DRAWFRAME, the contents of the window are copied rather than
		 * allowing the windows underneath to draw. This causes pixel
		 * corruption. The fix is to clear the SWP_DRAWFRAME bits.
		 */
		int bits = OS.GetWindowLong(handle, OS.GWL_EXSTYLE);
	 ...
	 * <code>setFullScreen(true)</code>, <code>setMaximized(true)</code>,
	 * <code>setMinimized(true)</code> and <code>setMaximumSize</code> will vary
	...
	public void setFullScreen(boolean fullScreen) {
		...
		if ((style & mask) != 0) {
			if (fullScreen) {
				styleFlags &= ~(OS.WS_CAPTION | OS.WS_MAXIMIZEBOX
						| OS.WS_MINIMIZEBOX | OS.WS_THICKFRAME);
			} else {
	...
	public void setImeInputMode(int mode) {
		...
		if (imeOn) {
			int[] lpfdwConversion = new int[1], lpfdwSentence = new int[1];
			if (OS.ImmGetConversionStatus(hIMC, lpfdwConversion,
					lpfdwSentence)) {
				int newBits = 0;
				...
				OS.ImmSetConversionStatus(hIMC, lpfdwConversion[0],
						lpfdwSentence[0]);
			}
		}
		OS.ImmReleaseContext(handle, hIMC);
	 ...
	 *
	 * @param width
	 *            the new maximum width for the receiver
	 * @param height
	 *            the new maximum height for the receiver
	 *
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	...
	public void setMaximumSize(int width, int height) {
		checkWidget();
		int zoom = getZoom();
		setMaximumSizeInPixels(DPIUtil.scaleUp(width, zoom),
				DPIUtil.scaleUp(height, zoom));
	}
	 ...
	 *
	 * @param size
	 *            the new maximum size for the receiver
	 *
	 * @exception IllegalArgumentException
	 *                <ul>
	...
	public void setMinimumSize(int width, int height) {
		checkWidget();
		int zoom = getZoom();
		setMinimumSizeInPixels(DPIUtil.scaleUp(width, zoom),
				DPIUtil.scaleUp(height, zoom));
	}
	 ...
	 * region.
	 * <p>
	 * NOTE: This method also sets the size of the shell. Clients should not
	 * call {@link #setSize} or {@link #setBounds} on this shell. Furthermore,
	 * the passed region should not be modified any more.
	 * </p>
	 *
	 * @param region
	...
	void setToolTipText(long hwnd, String text) {
		...
		// there
		// will be a native crash.
		// We only want to check visible, so set to NULL.
		lpti.lpszText = 0;

		// Updating invisible tooltip reportedly causes issues, see Bug 495473
		boolean isCurrentlyVisible = (OS.SendMessage(hwndToolTip,
				OS.TTM_GETCURRENTTOOL, 0, lpti) != 0) && (lpti.uId == hwnd);
	...
	void setToolTipText(NMTTDISPINFO lpnmtdi, char[] buffer) {
		/*
		 * Ensure that the current position of the mouse is inside the client
		 * area of the shell. This prevents tool tips from popping up over the
		 * shell trimmings.
		 */
		if (!hasCursor())
			return;
	...
	void setToolTipTitle(long hwndToolTip, String text, int icon) {
		/*
		 * Bug in Windows. For some reason, when TTM_SETTITLE is used to set the
		 * title of a tool tip, Windows leaks GDI objects. This happens even
		 * when TTM_SETTITLE is called with TTI_NONE and NULL. The documentation
		 * states that Windows copies the icon and that the programmer must free
		 * the copy but does not provide API to get the icon. For example, when
		 * TTM_SETTITLE is called with ICON_ERROR, when TTM_GETTITLE is used to
		 * query the title and the icon, the uTitleBitmap field in the
		 * TTGETTITLE struct is zero. The fix is to remember these values, only
		 * set them when then change and leak less.
		 *
		 * NOTE: This only happens on Vista.
		 */
		if (hwndToolTip != toolTipHandle && hwndToolTip != balloonTipHandle
				&& hwndToolTip != menuItemToolTipHandle) {
			return;
		}
		if (hwndToolTip == toolTipHandle
				|| hwndToolTip == menuItemToolTipHandle) {
			if (text == toolTitle
					|| (toolTitle != null && toolTitle.equals(text))) {
				if (icon == toolIcon)
					return;
			}
			toolTitle = text;
			...
			if (hwndToolTip == balloonTipHandle) {
				if (text == balloonTitle || (balloonTitle != null
						&& balloonTitle.equals(text))) {
					if (icon == toolIcon)
						return;
				}
				balloonTitle = text;
		...
		if (text != null) {
			/*
			 * Feature in Windows. The text point to by pszTitle must not exceed
			 * 100 characters in length, including the null terminator.
			 */
			if (text.length() > 99)
	...
	public void setVisible(boolean visible) {
		checkWidget();
		/*
		 * Feature in Windows. When ShowWindow() is called used to hide a
		 * window, Windows attempts to give focus to the parent. If the parent
		 * is disabled by EnableWindow(), focus is assigned to another windows
		 * on the desktop. This means that if you hide a modal window before the
		 * parent is enabled, the parent will not come to the front. The fix is
		 * to change the modal state before hiding or showing a window so that
		 * this does not occur.
		 */
		...
		/*
		 * Bug in Windows. Calling ShowOwnedPopups() to hide the child windows
		 * of a hidden window causes the application to be deactivated. The fix
		 * is to call ShowOwnedPopups() to hide children before hiding the
		 * parent.
		 */
		...
		if (visible) {
			if (parent != null && (parent.state & FOREIGN_HANDLE) != 0) {
				...
				if ((style & OS.WS_EX_TOOLWINDOW) != 0) {
					OS.SetWindowLong(hwndParent, OS.GWL_EXSTYLE,
							style & ~OS.WS_EX_TOOLWINDOW);
					/*
					 * Bug in Windows. The window does not show in the task bar
					 * when WS_EX_TOOLWINDOW is removed after the window has
					 * already been shown. The fix is to hide and shown the
					 * shell.
					 */
	...
	void subclass() {
		...
		if (ToolTipProc != 0) {
			...
			if (menuItemToolTipHandle != 0) {
				OS.SetWindowLongPtr(menuItemToolTipHandle, OS.GWLP_WNDPROC,
						newProc);
			}
		}
	}
	...
	void unsubclass() {
		...
		if (ToolTipProc != 0) {
			if (toolTipHandle != 0) {
				OS.SetWindowLongPtr(toolTipHandle, OS.GWLP_WNDPROC,
						ToolTipProc);
			}
			if (toolTipHandle != 0) {
				OS.SetWindowLongPtr(toolTipHandle, OS.GWLP_WNDPROC,
	...
	int widgetExtStyle() {
		...
		/*
		 * Feature in Windows. When a window that does not have a parent is
		 * created, it is automatically added to the Windows Task Bar, even when
		 * it has no title. The fix is to use WS_EX_TOOLWINDOW which does not
		 * cause the window to appear in the Task Bar.
		 */
		if (parent == null) {
	...
	Rectangle getClientRectInWindow() {
		...
		return new Rectangle(clientWindowLT.x + clientRect.left,
	...
	long windowProc(long hwnd, int msg, long wParam, long lParam) {
		if (handle == 0)
			return 0;
		if ((style & SWT.NO_MOVE) != 0 && msg == OS.WM_NCLBUTTONDOWN
				&& wParam == OS.HTCAPTION)
			return 0;
		if (hwnd == toolTipHandle || hwnd == balloonTipHandle
				|| hwnd == menuItemToolTipHandle) {
	...
	int widgetStyle() {
		 ...
		 * CreateWindowEx () will respect CW_USEDEFAULT and set the default
		 * window location and size.
		 *
		 * NOTE: When a WS_OVERLAPPED window is created, Windows gives the new
		 * window WS_CAPTION style bits. These two constants are as follows:
		 *
	...
	LRESULT WM_ACTIVATE(long wParam, long lParam) {
		...
		if (OS.LOWORD(wParam) == 0) {
			if (lParam == 0
					|| (lParam != toolTipHandle && lParam != balloonTipHandle
	...
	LRESULT WM_PAINT(long wParam, long lParam) {
		...
		if (!redrawTriggered)
			return new LRESULT(
					OS.DefWindowProc(hwnd, (int) OS.WM_PAINT, wParam, lParam));
		...
		// if you want to handle the redraw for yourself, remove the redraw
		// here!!
		redraw();
	...
	LRESULT WM_DESTROY(long wParam, long lParam) {
		...
		/*
		 * When the shell is a WS_CHILD window of a non-SWT window, the destroy
		 * code does not get called because the non-SWT window does not call
		 * dispose (). Instead, the destroy code is called here in WM_DESTROY.
		 */
		int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
	...
	LRESULT WM_MOUSEACTIVATE(long wParam, long lParam) {
		...
		/*
		 * Check for WM_MOUSEACTIVATE when an MDI shell is active and stop the
		 * normal shell activation but allow the mouse down to be delivered.
		 */
		...
		/*
		 * Get the current location of the cursor, not the location of the
		 * cursor when the WM_MOUSEACTIVATE was generated. This is strictly
		 * incorrect but is necessary in order to support Activate and
		 * Deactivate events for embedded widgets that have their own event
		 * loop. In that case, the cursor location reported by GetMessagePos()
		 * is the one for our event loop, not the embedded widget's event loop.
		 */
		POINT pt = new POINT();
		...
		/*
		 * When a shell is created with SWT.ON_TOP and SWT.NO_FOCUS, do not
		 * activate the shell when the user clicks on the the client area or on
		 * the border or a control within the shell that does not take focus.
		 */
	...
	LRESULT WM_NCHITTEST(long wParam, long lParam) {
		...
		if (!isEnabled() || !isActive()) {
			...
			long hittest = callWindowProc(handle, OS.WM_NCHITTEST, wParam,
					lParam);
	...
	LRESULT WM_NCLBUTTONDOWN(long wParam, long lParam) {
		 ...
		 * because the active shell was an MDI shell, set the active window to
		 * the top level shell but lock the active window and stop focus
		 * changes. This allows the user to interact the top level shell in the
		 * normal manner.
		 */
	...
	LRESULT WM_SETCURSOR(long wParam, long lParam) {
		/*
		 * Feature in Windows. When the shell is disabled by a Windows standard
		 * dialog (like a MessageBox or FileDialog), clicking in the shell does
		 * not bring the shell or the dialog to the front. The fix is to detect
		 * this case and bring the shell forward.
		 */
		...
		/*
		 * When the shell that contains a cursor is disabled, WM_SETCURSOR is
		 * called with HTERROR. Normally, when a control is disabled, the parent
		 * will get mouse and cursor events. In the case of a disabled shell,
		 * there is no enabled parent. In order to show the cursor when a shell
		 * is disabled, it is necessary to override WM_SETCURSOR when called
		 * with HTERROR to set the cursor but only when the mouse is in the
		 * client area of the shell.
		 */
		int hitTest = (short) OS.LOWORD(lParam);
	...
	LRESULT WM_SHOWWINDOW(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Bug in Windows. If the shell is hidden while the parent is iconic,
		 * Windows shows the shell when the parent is deiconified. This does not
		 * happen if the shell is hidden while the parent is not an icon. The
		 * fix is to track visible state for the shell and refuse to show the
		 * shell when the parent is shown.
		 */
	...
	LRESULT WM_WINDOWPOSCHANGING(long wParam, long lParam) {
		...
		if ((lpwp.flags & OS.SWP_NOSIZE) == 0) {
			...
			if ((style & SWT.NO_TRIM) == 0 && (style & trim) != 0) {
				if ((style & SWT.RESIZE) != 0) {
					lpwp.cy = Math.max(lpwp.cy,
							getSystemMetrics(OS.SM_CYMINTRACK));
				} else {
					RECT rect = new RECT();
					int bits1 = OS.GetWindowLong(handle, OS.GWL_STYLE);
					int bits2 = OS.GetWindowLong(handle, OS.GWL_EXSTYLE);
	...
	void setupOpenGl(GLData data) {
		...
		pfd.cAccumBits = (byte) (pfd.cAccumRedBits + pfd.cAccumGreenBits
	...
	public GLData getGLData() {
		...
		PIXELFORMATDESCRIPTOR pfd = new PIXELFORMATDESCRIPTOR();
		long hDC = OS.GetDC(handle);
		WGL.DescribePixelFormat(hDC, pixelFormat, PIXELFORMATDESCRIPTOR.sizeof,
				pfd);
		OS.ReleaseDC(handle, hDC);
	...
	/**
	 * Sets the OpenGL skijaContext associated with this GLCanvas to be the
	 * current GL skijaContext.
	 *
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
