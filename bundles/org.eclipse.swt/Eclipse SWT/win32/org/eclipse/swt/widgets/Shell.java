...
/**
 * Instances of this class represent the "windows" which the desktop or "window
 * manager" is managing. Instances that do not have a parent (that is, they are
 * built using the constructor, which takes a <code>Display</code> as the
 * argument) are described as <em>top level</em> shells. Instances that do have
 * a parent are described as <em>secondary</em> or <em>dialog</em> shells.
 * <p>
 * Instances are always displayed in one of the maximized, minimized or normal
 * states:
 * </p>
 * <ul>
 * <li>When an instance is marked as <em>maximized</em>, the window manager will
 ...
 * <li>When an instance is in the <em>normal</em> state (neither maximized or
 ...
 * below).</li>
 * <li>When an instance has been marked as <em>minimized</em>, its contents
 * (client area) will usually not be visible, and depending on the window
 * manager, it may be "iconified" (that is, replaced on the desktop by a small
 * simplified representation of itself), relocated to a distinguished area of
 * the screen, or hidden. Combinations of these changes are also possible.</li>
 * </ul>
 * <p>
 ...
 * <dl>
 * <dt><b>Styles:</b></dt>
 * <dd>BORDER, CLOSE, MIN, MAX, NO_MOVE, NO_TRIM, RESIZE, TITLE, ON_TOP, TOOL,
 * SHEET</dd>
 ...
 * <dd>the result of combining the constants which are required to produce a
 * typical application top level shell: (that is,
 * <code>CLOSE | TITLE | MIN | MAX | RESIZE</code>)</dd>
 * <dt><code>DIALOG_TRIM</code></dt>
 * <dd>the result of combining the constants which are required to produce a
 * typical application dialog shell: (that is,
 * <code>TITLE | CLOSE | BORDER</code>)</dd>
 * </dl>
 * <p>
...
public class Shell extends Decorations {
	...
	final static int[] SYSTEM_COLORS = { OS.COLOR_BTNFACE, OS.COLOR_WINDOW, OS.COLOR_BTNTEXT, OS.COLOR_WINDOWTEXT,
			OS.COLOR_HIGHLIGHT, OS.COLOR_SCROLLBAR, };
	...
	/**
	 * Constructs a new instance of this class given only the style value describing
	 * its behavior and appearance. This is equivalent to calling
	...
	/**
	 * Constructs a new instance of this class given only the display to create it
	 * on. It is created with style <code>SWT.SHELL_TRIM</code>.
	 * <p>
	 * Note: Currently, null can be passed in for the display argument. This has the
	 * effect of creating the shell on the currently active display if there is one.
	 * If there is no current display, the shell is created on a "default" display.
	 * <b>Passing in null as the display argument is not considered to be good
	...
	private void onResize(Event e) {
		...
		if (SWT.USE_SKIJA) {
			...
			if (SWT.USE_OPENGL)
				...
				surface = Surface.makeRaster(ImageInfo.makeN32Premul(rect.width, rect.height), 0,
	...
	/**
	 * Constructs a new instance of this class given only its parent. It is created
	 * with style <code>SWT.DIALOG_TRIM</code>.
	 * <p>
	 * Note: Currently, null can be passed in for the parent. This has the effect of
	 * creating the shell on the currently active display if there is one. If there
	 * is no current display, the shell is created on a "default" display.
	 * <b>Passing in null as the parent is not considered to be good coding style,
	 ...
	 * <code>Shell</code>. It is marked public only so that it can be shared within
	...
	/**
	 * Invokes platform specific functionality to allocate a new shell that is not
	 * embedded.
	 * <p>
	 * <b>IMPORTANT:</b> This method is <em>not</em> part of the public API for
	 * <code>Shell</code>. It is marked public only so that it can be shared within
	 ...
	 * operations are performed on the receiver, by sending the listener one of the
	 * messages defined in the <code>ShellListener</code> interface.
	 *
	...
	/**
	 * Requests that the window manager close the receiver in the same way it would
	 * be closed when the user clicks on the "close box" or performs some other
	 * platform specific key or mouse combination that indicates the window should
	 * be removed.
	 *
	 * @exception SWTException
	 *                         <ul>
	...
	void createBalloonTipHandle() {
		balloonTipHandle = OS.CreateWindowEx(0, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null,
				OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX | OS.TTS_BALLOON, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, handle,
				0, OS.GetModuleHandle(null), null);
		if (balloonTipHandle == 0)
		 ...
		 * the tooltip will not honour the new line unless TTM_SETMAXTIPWIDTH is set.
		 * The fix is to set TTM_SETMAXTIPWIDTH to a large value.
		 */
	...
	void setTitleColoring() {
		...
		if (OS.WIN32_BUILD >= OS.WIN32_BUILD_WIN10_2004) {
			// Documented since build 20348, but was already present since build
			// 19041
	...
	void createHandle() {
		...
		/*
		 * On Windows 98 and NT, setting a window to be the top most window using
		 * HWND_TOPMOST can result in a parent dialog shell being moved behind its
		 * parent if the dialog has a sibling that is currently on top This only occurs
		 * using SetWindowPos (), not when the handle is created.
		 */
		/*
		...
		if (!embedded) {
			...
			/*
			 * Bug in Windows. When the WS_CAPTION bits are cleared using SetWindowLong(),
			 * Windows does not resize the client area of the window to get rid of the
			 * caption until the first resize. The fix is to use SetWindowPos() with
			 * SWP_DRAWFRAME to force the frame to be redrawn and resized.
			 */
	...
	long createToolTipHandle(long parent) {
		long toolTipHandle = OS.CreateWindowEx(0, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null,
				OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, parent, 0,
	...
	void fixActiveShell() {
		/*
		 * Feature in Windows. When the active shell is disposed or hidden, Windows
		 * normally makes the parent shell active and assigns focus. This does not
		 * happen when the parent shell is disabled. Instead, Windows assigns focus to
		 * the next shell on the desktop (possibly a shell in another application). The
		 * fix is to activate the disabled parent shell before disposing or hiding the
		 * active shell.
		 */
	...
	void fixToolTip() {
		/*
		 * Bug in Windows. On XP, when a tooltip is hidden due to a time out or mouse
		 * press, the tooltip remains active although no longer visible and won't show
		 * again until another tooltip becomes active. If there is only one tooltip in
		 * the window, it will never show again. The fix is to remove the current
		 * tooltip and add it again every time the mouse leaves the control.
		 */
		if (toolTipHandle == 0)
			return;
		TOOLINFO lpti = new TOOLINFO();
	 ...
	 * display on which it was created (so that all other shells on that display,
	 * which are not the receiver's children will be drawn behind it) and forces the
	...
	/**
	 * Returns <code>true</code> if the receiver is currently in fullscreen state,
	 * and false otherwise.
	 *
	...
	/**
	 * Returns the receiver's input method editor mode. This will be the result of
	 * bitwise OR'ing together one or more of the following constants defined in
	 * class <code>SWT</code>: <code>NONE</code>, <code>ROMAN</code>,
	...
	/**
	 * Returns a point describing the maximum receiver's size. The x coordinate of
	 * the result is the maximum width of the receiver. The y coordinate of the
	 * result is the maximum height of the receiver.
	 *
	...
	/**
	 * Returns the region that defines the shape of the shell, or <code>null</code>
	...
	/**
	 * Returns a ToolBar object representing the tool bar that can be shown in the
	 * receiver's trim. This will return <code>null</code> if the platform does not
	 * support tool bars that are not part of the content area of the shell, or if
	 * the Shell's style does not support having a tool bar.
	 *
	...
	long hwndMDIClient() {
		if (hwndMDIClient == 0) {
			...
			hwndMDIClient = OS.CreateWindowEx(0, new TCHAR(0, "MDICLIENT", true), null, widgetStyle, 0, 0, 0, 0, handle,
					0, OS.GetModuleHandle(null), new CREATESTRUCT());
	...
	/**
	 * Moves the receiver to the top of the drawing order for the display on which
	 * it was created (so that all other shells on that display, which are not the
	 * receiver's children will be drawn behind it), marks it visible, sets the
	...
	public void open() {
		...
		if (isDisposed())
			return;
		/*
		 * Bug in Windows XP. Despite the fact that an icon has been set for a window,
		 * the task bar displays the wrong icon the first time the window is made
		 * visible with ShowWindow() after a call to BringToTop(), when a long time
		 * elapses between the ShowWindow() and the time the event queue is read. The
		 * icon in the window trimming is correct but the one in the task bar does not
		 * get updated. The fix is to call PeekMessage() with the flag PM_NOREMOVE and
		 * PM_QS_SENDMESSAGE to respond to a cross thread WM_GETICON.
		 *
		 * NOTE: This allows other cross thread messages to be delivered, most notably
		 * WM_ACTIVATE.
		 */
		MSG msg = new MSG();
		...
		/*
		 * When no widget has been given focus, or another push button has focus, give
		 * focus to the default button. This avoids overriding the default button.
		 */
	 ...
	 * when operations are performed on the receiver.
	 *
	 ...
	 * If the receiver is visible, moves it to the top of the drawing order for the
	 * display on which it was created (so that all other shells on that display,
	 * which are not the receiver's children will be drawn behind it) and asks the
	 * window manager to make the shell active
	 *
	 * @exception SWTException
	 *                         <ul>
	...
	void setActiveControl(Control control, int type) {
		...
		/*
		 * Compute the list of controls to be activated and deactivated by finding the
		 * first common parent control.
		 */
		...
		/*
		 * It is possible (but unlikely), that application code could have destroyed
		 * some of the widgets. If this happens, keep processing those widgets that are
		 * not disposed.
		 */
	...
	public Rectangle getBounds() {
		if (getDisplay().isRescalingAtRuntime()) {
			...
			return display.translateRectangleInPointsInDisplayCoordinateSystemByContainment(boundsInPixels.x,
					boundsInPixels.y, boundsInPixels.width, boundsInPixels.height);
		}
	...
	public void setBounds(int x, int y, int width, int height) {
		if (getDisplay().isRescalingAtRuntime()) {
			Rectangle boundsInPixels = display.translateRectangleInPixelsInDisplayCoordinateSystemByContainment(x, y,
					width, height);
	...
	void setBoundsInPixels(int x, int y, int width, int height, int flags, boolean defer) {
		if (fullScreen)
			setFullScreen(false);
		/*
		 * Bug in Windows. When a window has alpha and SetWindowPos() is called with
		 * SWP_DRAWFRAME, the contents of the window are copied rather than allowing the
		 * windows underneath to draw. This causes pixel corruption. The fix is to clear
		 * the SWP_DRAWFRAME bits.
		 */
		int bits = OS.GetWindowLong(handle, OS.GWL_EXSTYLE);
	 ...
	 * <code>setMaximumSize</code> will vary by platform. Typically, the behavior
	 * will match the platform user's expectations, but not always. This should be
	 * avoided if possible.
	 * </p>
	 *
	 ...
	 * of bitwise OR'ing together one or more of the following constants defined in
	 * class <code>SWT</code>: <code>NONE</code>, <code>ROMAN</code>,
	 * <code>DBCS</code>, <code>PHONETIC</code>, <code>NATIVE</code>,
	 * <code>ALPHA</code>.
	 *
	...
	/**
	 * Sets the receiver's maximum size to the size specified by the arguments. If
	 * the new maximum size is smaller than the current size of the receiver, the
	 * receiver is resized to the new maximum size.
	 * <p>
	 * Note: The result of intermixing calls to <code>setMaximumSize</code> and
	 * <code>setFullScreen(true)</code> will vary by platform. Typically, the
	...
	/**
	 * Sets the receiver's maximum size to the size specified by the argument. If
	 * the new maximum size is smaller than the current size of the receiver, the
	 * receiver is resized to the new maximum size.
	 * <p>
	 * Note: The result of intermixing calls to <code>setMaximumSize</code> and
	...
	/**
	 * Sets the receiver's minimum size to the size specified by the arguments. If
	 * the new minimum size is larger than the current size of the receiver, the
	 * receiver is resized to the new minimum size.
	 *
	...
	/**
	 * Sets the receiver's minimum size to the size specified by the argument. If
	 * the new minimum size is larger than the current size of the receiver, the
	 * receiver is resized to the new minimum size.
	 *
	...
	/**
	 * Sets the shape of the shell to the region specified by the argument. When the
	 * argument is null, the default shape of the shell is restored. The shell must
	 * be created with the style SWT.NO_TRIM in order to specify a region.
	 * <p>
	 * NOTE: This method also sets the size of the shell. Clients should not call
	 * {@link #setSize} or {@link #setBounds} on this shell. Furthermore, the passed
	 * region should not be modified any more.
	 * </p>
	 *
	...
	void setToolTipText(long hwnd, String text) {
		...
		// Previous `TTM_GETTOOLINFO` was sent to specific tool, which is likely
		// set to `LPSTR_TEXTCALLBACK`.
		// In this case, WINAPI will set `TOOLINFO.lpszText` to
		// `LPSTR_TEXTCALLBACK` (numerically -1).
		// Now we're sending `TTM_GETCURRENTTOOL`, which may be sent to a
		// different tool.
		// If it happens to have text other than `LPSTR_TEXTCALLBACK`, WINAPI
		// will try to copy it for us.
		// `TOOLINFO.lpszText` must be either NULL or point to valid buffer, or
		// there will be a native crash.
		// We only want to check visible, so set to NULL.
		lpti.lpszText = 0;

		// Updating invisible tooltip reportedly causes issues, see Bug 495473
		boolean isCurrentlyVisible = (OS.SendMessage(hwndToolTip, OS.TTM_GETCURRENTTOOL, 0, lpti) != 0)
				&& (lpti.uId == hwnd);
	...
	void setToolTipText(NMTTDISPINFO lpnmtdi, char[] buffer) {
		/*
		 * Ensure that the current position of the mouse is inside the client area of
		 * the shell. This prevents tool tips from popping up over the shell trimmings.
		 */
		if (!hasCursor())
			return;
		long hHeap = OS.GetProcessHeap();
	...
	void setToolTipTitle(long hwndToolTip, String text, int icon) {
		/*
		 * Bug in Windows. For some reason, when TTM_SETTITLE is used to set the title
		 * of a tool tip, Windows leaks GDI objects. This happens even when TTM_SETTITLE
		 * is called with TTI_NONE and NULL. The documentation states that Windows
		 * copies the icon and that the programmer must free the copy but does not
		 * provide API to get the icon. For example, when TTM_SETTITLE is called with
		 * ICON_ERROR, when TTM_GETTITLE is used to query the title and the icon, the
		 * uTitleBitmap field in the TTGETTITLE struct is zero. The fix is to remember
		 * these values, only set them when then change and leak less.
		 *
		...
		if (text != null) {
			/*
			 * Feature in Windows. The text point to by pszTitle must not exceed 100
	...
	public void setVisible(boolean visible) {
		checkWidget();
		/*
		 * Feature in Windows. When ShowWindow() is called used to hide a window,
		 * Windows attempts to give focus to the parent. If the parent is disabled by
		 * EnableWindow(), focus is assigned to another windows on the desktop. This
		 * means that if you hide a modal window before the parent is enabled, the
		 * parent will not come to the front. The fix is to change the modal state
		...
		/*
		 * Bug in Windows. Calling ShowOwnedPopups() to hide the child windows of a
		 * hidden window causes the application to be deactivated. The fix is to call
		 * ShowOwnedPopups() to hide children before hiding the parent.
		 */
		...
		if (visible) {
			if (parent != null && (parent.state & FOREIGN_HANDLE) != 0) {
				...
				if ((style & OS.WS_EX_TOOLWINDOW) != 0) {
					 ...
					 * WS_EX_TOOLWINDOW is removed after the window has already been shown. The fix
					 * is to hide and shown the shell.
					 */
	...
	int widgetExtStyle() {
		...
		/*
		 * Feature in Windows. When a window that does not have a parent is created, it
		 * is automatically added to the Windows Task Bar, even when it has no title.
		 * The fix is to use WS_EX_TOOLWINDOW which does not cause the window to appear
		 * in the Task Bar.
		 */
		if (parent == null) {
	...
	Rectangle getClientRectInWindow() {
		...
		return new Rectangle(clientWindowLT.x + clientRect.left, clientWindowLT.y + clientRect.top,
	...
	int widgetStyle() {
		...
		/*
		 * Use WS_OVERLAPPED for all windows, either dialog or top level so that
		 * CreateWindowEx () will respect CW_USEDEFAULT and set the default window
		 * location and size.
		 *
		 * NOTE: When a WS_OVERLAPPED window is created, Windows gives the new window
		 * WS_CAPTION style bits. These two constants are as follows:
		 *
		 * WS_OVERLAPPED = 0 WS_CAPTION = WS_BORDER | WS_DLGFRAME
		 */
	...
	LRESULT WM_ACTIVATE(long wParam, long lParam) {
		...
		if (OS.LOWORD(wParam) == 0) {
			if (lParam == 0
					|| (lParam != toolTipHandle && lParam != balloonTipHandle && lParam != menuItemToolTipHandle)) {
	...
	LRESULT WM_DESTROY(long wParam, long lParam) {
		...
		/*
		 * When the shell is a WS_CHILD window of a non-SWT window, the destroy code
		 * does not get called because the non-SWT window does not call dispose ().
		 * Instead, the destroy code is called here in WM_DESTROY.
		 */
		int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
	...
	LRESULT WM_GETMINMAXINFO(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		if (minWidth != SWT.DEFAULT || minHeight != SWT.DEFAULT || maxWidth != SWT.DEFAULT
				|| maxHeight != SWT.DEFAULT) {
	...
	LRESULT WM_MOUSEACTIVATE(long wParam, long lParam) {
		...
		/*
		 * Check for WM_MOUSEACTIVATE when an MDI shell is active and stop the normal
		 * shell activation but allow the mouse down to be delivered.
		 */
		...
		/*
		 * Get the current location of the cursor, not the location of the cursor when
		 * the WM_MOUSEACTIVATE was generated. This is strictly incorrect but is
		 * necessary in order to support Activate and Deactivate events for embedded
		 * widgets that have their own event loop. In that case, the cursor location
		 * reported by GetMessagePos() is the one for our event loop, not the embedded
		 * widget's event loop.
		 */
		POINT pt = new POINT();
		...
		/*
		 * When a shell is created with SWT.ON_TOP and SWT.NO_FOCUS, do not activate the
		 * shell when the user clicks on the the client area or on the border or a
		 * control within the shell that does not take focus.
		 */
	...
	LRESULT WM_NCLBUTTONDOWN(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * When the normal activation was interrupted in WM_MOUSEACTIVATE because the
		 * active shell was an MDI shell, set the active window to the top level shell
		 * but lock the active window and stop focus changes. This allows the user to
		 * interact the top level shell in the normal manner.
		 */
	...
	LRESULT WM_SETCURSOR(long wParam, long lParam) {
		/*
		 * Feature in Windows. When the shell is disabled by a Windows standard dialog
		 * (like a MessageBox or FileDialog), clicking in the shell does not bring the
		 * shell or the dialog to the front. The fix is to detect this case and bring
		 * the shell forward.
		 */
		...
		/*
		 * When the shell that contains a cursor is disabled, WM_SETCURSOR is called
		 * with HTERROR. Normally, when a control is disabled, the parent will get mouse
		 * and cursor events. In the case of a disabled shell, there is no enabled
		 * parent. In order to show the cursor when a shell is disabled, it is necessary
		 * to override WM_SETCURSOR when called with HTERROR to set the cursor but only
		 * when the mouse is in the client area of the shell.
		 */
		int hitTest = (short) OS.LOWORD(lParam);
	...
	LRESULT WM_SHOWWINDOW(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Bug in Windows. If the shell is hidden while the parent is iconic, Windows
		 * shows the shell when the parent is deiconified. This does not happen if the
		 * shell is hidden while the parent is not an icon. The fix is to track visible
		 * state for the shell and refuse to show the shell when the parent is shown.
		 */
	...
	void setupOpenGl(GLData data) {

		getDisplay().setData(USE_OWNDC_KEY, false);
		if (data == null)
		...
		openGLcontext = WGL.wglCreateContext(hDC);
		if (openGLcontext == 0) {
			OS.ReleaseDC(handle, hDC);
		...
		// What is shareContext?? Check use case for SWT widget toolkit.
		// TODO check whether this is necessary...
		if (data.shareContext != null) {
			WGL.wglShareLists(data.shareContext.getContext(), openGLcontext);
		}

		Listener listener = e -> {
			switch (e.type) {
			case SWT.Dispose:
				WGL.wglDeleteContext(openGLcontext);
				break;
			}
		};
		addListener(SWT.Dispose, listener);
	...
	/**
	 * Returns a GLData object describing the created openGlcontext.
	 *
	 * @return GLData description of the OpenGL openGlcontext attributes
	 * @exception SWTException
	 *                         <ul>
	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                         disposed</li>
	...
	/**
	 * Returns a boolean indicating whether the receiver's OpenGL skijaContext is
	 * the current skijaContext.
	 *
	 * @return true if the receiver holds the current OpenGL skijaContext, false
	 *         otherwise
	 * @exception SWTException
	 *                         <ul>
	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	...
	public boolean isCurrent() {
		checkWidget();
		return WGL.wglGetCurrentContext() == openGLcontext;
	}

	/**
	 * Sets the OpenGL skijaContext associated with this GLCanvas to be the current
	 * GL skijaContext.
	 *
	 * @exception SWTException
	 *                         <ul>
	...
	public void setCurrent() {
		checkWidget();
		if (WGL.wglGetCurrentContext() == openGLcontext)
			return;
		long hDC = OS.GetDC(handle);
		WGL.wglMakeCurrent(hDC, openGLcontext);
		OS.ReleaseDC(handle, hDC);
	}
